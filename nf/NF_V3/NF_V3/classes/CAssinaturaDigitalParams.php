...
*/
     //Este comando é necessário para ir até o diretório do programa 
    ...
    // Variaveis para uso das funcoes de assinatura
        ...
        // /var/www/html/nf/NF_V3/NF_V3/classes/CAssinaturaDigitalParams.php /home/felipe/nfse/nfse.xml 84818889000109 '' LoteRps
        ...
        // Carregamento de dados do certificado
            ...
            echo $this->config['dados']."config_cliente/".$this->cnpj.".ini";
        ...
        */
        private function loadCerts(){
            ...
            if (!openssl_pkcs12_read($key,$x509certdata,$this->senhaPFX)){
                echo  "CAssinaturaDigital->loadCerts{ Certificado digital nao pode ser lido, verifique se a senha está correta }";
...
$assinatura = new CAssinaturaDigital($xmlPath, $cnpj);

// Grava no arquivo se estiver tudo ok. Retorna o erro se der errado.
if ($assinatura->assinarXml( $tag1, $tag2 )) {
    //$assinatura->salvar();
}else{
    echo ($assinatura->getErro());
    // echo "Bahh";
