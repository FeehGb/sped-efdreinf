...
def makedirs(*path_dirs):
    ...
    for path in path_dirs :
        try:
            ...
            print("Diretorio {0} nao criado\nError:{1}".format(path, error))
            pass
...
# se estiver rodando um teste
    ...
    estado     = 'BR'
    xmlPath    = '/user/transf'
    jsonConfig = '{"versao":"3.00","dir_retorno":"\/user\/transf\/","dir_procCTe":"\/user\/transf\/","dir_procEventoCTe":"\/user\/transf\/"}'
...
makedirs(jsonConfig['dir_procCTe'],jsonConfig['dir_procEventoCTe'])
...
for parte in partes:
    ...
    if schema == 'procCTe_v3.00.xsd':
        ...
        retorno = [
            ...
            getXpath('procEventoCTe/retEventoCTe/infEvento/xEvento'                              , unzipedXML) , # 05  WS-D-R-3-DESC-EVENTO       PIC  X(060). 
            getXpath('procEventoCTe/retEventoCTe/infEvento/dhRegEvento'                          , unzipedXML) , # 05  WS-D-R-3-DH-AUTORIZ        PIC  9(014). 
            getXpath('procEventoCTe/retEventoCTe/infEvento/nProt'                                , unzipedXML) , # 05  WS-D-R-3-PROTOCOLO         PIC  9(015). 
        ...
        #if(chave =='35211160526977001906570100000478461007025082'):
        #    print('achou')
    ...
    if retornoCob != 'debug':
        if schema.strip():
            ...
            path = jsonConfig['dir_retorno'] + 'NAO_CATEGORIZADO.txt'
