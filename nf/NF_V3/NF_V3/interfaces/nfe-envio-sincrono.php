<?php 
    /*
        ...
        Modo de uso: 
            ...
            php /var/www/html/nf/NF_V3/NF_V3/interfaces/nfe-envio-sincrono.php /var/www/html/nf/NF_V3/NF_V3/interfaces/lixos/testeassincronoSefaz.txt debug PR PR -1 1
            php /var/www/html/nf/NF_V3/NF_V3/interfaces/nfe-envio.php /user/nfe/35561071000148/CaixaEntrada/Processar/NFC-000020216-P1-001466-20210827-135216.TXT  debug  41  PR
    */
    ...
    if (!isset($argv[1])){
        echo "
            ENTRADA:
                ...
                argv[5]*= sincrono              |      '1' ; 0 - assincrono - 1 sincrono
                argv[6]*= recuperarChave        |     '-1' ; 
            ...
            Retorno Sincrono -> 
                ...
                WS-203-AMBIENTE           
                WS-203-UF-PROCESSOU       
                WS-203-DATA-PROCESSAMENTO 
                WS-203-PROTOCOLO-SEFAZ    
                WS-203-STATUS-SEFAZ       
                WS-203-MOTIVO             
                WS-203-NRO-PROT-PROCESSADO
                WS-203-XML-INTEGRAL.  
    ...
    $sincrono            = (isset($argv[5])?$argv[5]:'1')    ; // Esse arquivo sempre forca o sincrono
    $recuperarChave      = (isset($argv[6])?$argv[6]:'-1')    ; 
    $testesAutomatizados = (isset($argv[7])?$argv[7]:'2')     ; 
    
    //var_dump($sincrono);exit();
    ...
    $arquivo_sefaz = $temp.$nome_raiz."enviado.xml";
    ...
    if ($retornoConversor !== true){
        ...
        $retorno[] = "";    // WS-203-CNPJ               
        $retorno[] = "";    // WS-203-UF                 
        $retorno[] = "";    // WS-203-CHAVE-NFE          
        $retorno[] = "";    // WS-203-AMBIENTE           
        $retorno[] = "";    // WS-203-UF-PROCESSOU       
        $retorno[] = "";    // WS-203-DATA-PROCESSAMENTO 
        $retorno[] = "";    // WS-203-PROTOCOLO-SEFAZ    
        $retorno[] = "-1";    // WS-203-STATUS-SEFAZ       
        $retorno[] = "$retornoConversor";    // WS-203-MOTIVO             
        $retorno[] = "";    // WE-RET-QRCODE            
        $retorno[] = "";    // WS-203-NRO-PROT-PROCESSADO
        $retorno[] = "";    // WS-203-XML-INTEGRAL.  
    ...
    $array_substituicao= array(
        ...
        'ALTERAR_DADOS_XML'=> str_replace('<?xml version="1.0" encoding="UTF-8"?>', '',file_get_contents($arquivo_xml)),
        'ALTERAR_DADOS_ID_LOTE'=>substr(str_replace(',', '', number_format(microtime(true)*1000000, 0)), 0, 15),
        'ALTERAR_INDSINC' => $sincrono,//$dados['nfe']['envioLoteSincrono'],
        'TAG VAZIA' =>'',
        "\n"=>'',
        "\r"=>'',
        "\t"=>'',
    ...
    */
    ...
    //var_dump($arquivo_txt_saida);exit();
    if (!$isOff){
        // TODO gravar o XML retorno 
        //echo "\n\n--------------------\n\n" ; 
        file_get_contents($arquivo_xml);
        //file_put_contents($arquivo_sefaz, $arquivo_xml);  
        ...
        //$chNFe      = getVlTag("chNFe")    ; // WS-203-CHAVE-NFE  
        $tpAmb      = getVlTag("tpAmb")    ; // WS-203-AMBIENTE 
        $cUF        = getVlTag("cUF")      ; // WS-203-UF-PROCESSOU 
        $dhRecbto   = getVlTag("dhRecbto") ; // WS-203-DATA-PROCESSAMENTO 
        $nProt      = getVlTag("nProt")    ; // WS-203-PROTOCOLO-SEFAZ 
        $cStat      = getVlTag("cStat")    ; // WS-203-STATUS-SEFAZ 
        $xMotivo    = getVlTag("xMotivo")  ; // WS-203-MOTIVO 
    ...
    $retorno[] = $chave;         // WS-203-CHAVE-NFE          
    $retorno[] = $tpAmb;         // WS-203-AMBIENTE           
    $retorno[] = $cUF;           // WS-203-UF-PROCESSOU       
    $retorno[] = $dhRecbto;      // WS-203-DATA-PROCESSAMENTO 
    $retorno[] = $nProt;         // WS-203-PROTOCOLO-SEFAZ    
    $retorno[] = $cStat;         // WS-203-STATUS-SEFAZ       
    $retorno[] = $xMotivo;       // WS-203-MOTIVO             
    $retorno[] = $qrCode;        // WE-RET-QRCODE            
    $retorno[] = $protocolo;     // WS-203-NRO-PROT-PROCESSADO
