<?php
     ...
     * @param ArrayObject{chave:String, rota:String} $uri = [
     *          'chave' => base da url do qrcode,
     *          'rota' => rota do qrcode, muda confome o estado e ambiente
     *      ]   
     * @return void
     */
    function nfe_qrcode2_gerar($xml_path, $uri, $tempDIr, $appendOnXml=false, $token)
    {
        ...
        $url = nfe_qrcode_createUrl($uri, $tempDIr, $token);
        
        // se for para anexar no xml
        if ($appendOnXml)
        {
            // insere a tag de qrcode no xml 
            nfe_qrcode_appendOnXml($url, $uri->chave);
     ...
     */
    function nfe_qrcode_createUrl($uri, $tempDIr, $token)
    {
        ...
        #$url  = "$uri/nfce/qrcode?p=$params|$hash";
        
        $url  = "$uri->chave$uri->rota?p=$params|$hash";
     ...
     * @return void
     */
    function nfe_qrcode_appendOnXml($url, $uri)
    {
        global $nfe_qrcode_xml;
        ...
        $urlChave = $nfe_qrcode_xml->createElement('urlChave', $uri);
