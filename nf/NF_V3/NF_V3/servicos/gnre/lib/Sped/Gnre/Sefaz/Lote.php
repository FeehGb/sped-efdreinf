...
class Lote extends LoteGnre
{
    ...
    public function getHeaderSoap()
    {
        $action = $this->ambienteDeTeste ?
            'https://www.testegnre.pe.gov.br/webservice/GnreRecepcaoLote' :
            'https://www.gnre.pe.gov.br/webservice/GnreRecepcaoLote';
    ...
    public function toXml2($xml)
    {
        ...
        $gnre->formatOutput = true;
        $gnre->preserveWhiteSpace = false;

        $this->getSoapEnvelop2($gnre, $xml);
    ...
    public function getSoapEnvelop2($gnre, $loteGnre)
    {
        ...
        $gnreCabecalhoSoap->setAttribute('xmlns', 'http://www.gnre.pe.gov.br/webservice/GnreLoteRecepcao');
        ...
        #$externalXML = simplexml_load_string($loteGnre);
        
        $TLote_GNRE = $gnre->createElement('TLote_GNRE');
        $TLote_GNRE ->setAttribute('versao',"2.00");
        $loteXmlns = $gnre->createAttribute('xmlns');
        $loteXmlns->value = 'http://www.gnre.pe.gov.br';
        
        $content = $gnre->createDocumentFragment();
        $content->appendXML($loteGnre->guias->asXML());
        
        $gnreDadosMsg = $gnre->createElement('gnreDadosMsg');
        $gnreDadosMsg->setAttribute('xmlns', $action);
        
        
        $TLote_GNRE->appendChild($loteXmlns); 
        $TLote_GNRE->appendChild($content);
        
        $gnreDadosMsg->appendChild($TLote_GNRE);
        #$gnreDadosMsg->nodeValue = $loteGnre;
