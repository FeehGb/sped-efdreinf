...
$senha = $argv[2];
$dados_cert = [];
$cnpj = explode('.', $certificado)[0];

$priKey = "{$cnpj}_priKey.pem";
$pubKey = "{$cnpj}_pubKey.pem";
$certKey = "{$cnpj}_certKey.pem";
...
if(file_exists($pubKey)){
	...
	if ( !substr($conteudo,0,100) == substr($dados_cert['cert'],0,100) ) {
		...
		//exec("openssl pkcs12 -in {$argv[1]} -nokeys -out {$cnpj}_sefaz_certKey.pem -password pass:{$senha}");
