<?php 
    /*
        Programa:  nfe-carta-correcao.php
        Descricão: Programa responsavel por realizar a correcao de nota na a sefaz
        Autor:     Fernando H. Crozetta (02/03/2018)
        Modo de uso: 
            php /var/www/html/nf/NF_V3/NF_V3/interfaces/nfe-carta-correcao.php <cnpj> <chNfe> <sequencia> "<correcao>" <dhEvento> <caminho-arquivo_saida.xml>  <tipo_ambiente> <UF> <autorizadora>
    */
    ...
    // main
    //cria_diretorios(basename($arquivo_txt_saida));    
    cria_diretorios($temp);
    
    // Carrega o xml e busca dados
    $dados            = parse_ini_file($config['dados']."/config_cliente/".$cnpj.".ini",true);
    $dados_ws         = new BuscaWebService($autorizadora,'nfe',$stringAmbiente);
    $array_webservice = $dados_ws->buscarServico("carta_correcao",$dados['nfe']['versao']);
    ...
    $array_substituicao= array(
        ...
        'ALTERAR_SEQUENCIA'=>$sequencia,
        'ALTERAR_XCORRECAO'=>$correcao,
        'ALTERAR_ID_EVENTO'=> "ID110110" . $chave . sprintf('%02d', $sequencia),
        'TAG VAZIA' =>'',
        "\n"=>'',
        "\r"=>'',
        "\t"=>'',
    ...
    $arquivo_validacao = $temp.$chave."correcao.". $sequencia .".validacao.xml";
    $arquivo_xml = $temp.$chave."correcao.". $sequencia .".xml";
    ...
    file_put_contents($temp.$chave.".carta_correcao_parte1.xml",$t);
    ...
    */
    ...
    file_put_contents($temp.$chave.".carta_correcao_parte2.xml",$retornoEvento);
