...
class ConvertMDFePHP
{
    ...
    /**
     * Versao do arquivo, para validacao
     */
    public $versao='';
        ...
        for ($l = 0; $l < count($arquivo); $l++) {
            ...
            switch ($dados[0]) {
                ...
                case "B": //identificadores [infMDFe]
                    ...
                    if ($dados[4] != 0) {
                        $tpTransp = $dom->createElement("tpTransp", $dados[4]);
                    ...
                    $nMDF = $dom->createElement("nMDF", ltrim($dados[7],0));
					$this->numero = ltrim($dados[7],0);
                    ...
                    $ide->appendChild($cDV);
                    $modal = $dom->createElement("modal", $dados[10]);		
                    $ide->appendChild($modal);
                    $dhEmi = $dom->createElement("dhEmi", $dados[11]);
                    $ide->appendChild($dhEmi);
                    $tpEmis = $dom->createElement("tpEmis", $dados[12]);
                    $ide->appendChild($tpEmis);
                    $procEmi = $dom->createElement("procEmi", $dados[13]);
                    $ide->appendChild($procEmi);
                    $verProc = $dom->createElement("verProc", $dados[14]);
                    $ide->appendChild($verProc);
                    $UFIni = $dom->createElement("UFIni", $dados[15]);
                    $ide->appendChild($UFIni);
                    $UFFim = $dom->createElement("UFFim", $dados[16]);
                    $ide->appendChild($UFFim);
					$dhIniViagem = $dom->createElement("dhIniViagem", $dados[17]);
                ...
                case "R":
                    ...
                    $cIntTracao = $dom->createElement("cInt", $dados[2]);
                    $veicTracao->appendChild($cIntTracao);
                    $placa = $dom->createElement("placa", $dados[3]);
                    $veicTracao->appendChild($placa);
					$RENAVAM = $dom->createElement("RENAVAM", $dados[4]);
                    $veicTracao->appendChild($RENAVAM);
                    $tara = $dom->createElement("tara", ltrim($dados[5],0));
                    $veicTracao->appendChild($tara);
					$capKG = $dom->createElement("capKG", ltrim($dados[6],0));
                    $veicTracao->appendChild($capKG);
					$capM3 = $dom->createElement("capM3", ltrim($dados[7],0));
                    $veicTracao->appendChild($capM3);
					
					if(trim($dados[8],0) != "" || trim($dados[9],0) != ""){
						$prop = $dom->createElement("prop");
						if(ltrim($dados[8],0) != ""){
							$CPF = $dom->createElement("CPF", $dados[8]);
						...
						$RNTRC = $dom->createElement("RNTRC", ltrim($dados[9],0));
						$prop->appendChild($RNTRC);
						$xNome = $dom->createElement("xNome", $dados[11]);
						$prop->appendChild($xNome);
						$IE = $dom->createElement("IE", $dados[12]);
						$prop->appendChild($IE);
						$UF = $dom->createElement("UF", $dados[13]);
						$prop->appendChild($UF);
						$tpProp = $dom->createElement("tpProp", $dados[14]);
						$prop->appendChild($tpProp);

						$veicTracao->appendChild($prop);
					}
					
                    $tpRod = $dom->createElement("tpRod", $dados[15]);
                    $veicTracao->appendChild($tpRod);
                    $tpCarTracao = $dom->createElement("tpCar", $dados[16]);
                    $veicTracao->appendChild($tpCarTracao);
                    $UF = $dom->createElement("UF", $dados[17]);
                    ...
                    break;
                case "R0":
                    // Este grupo pode ser 0-n vezes, mas segundo o reoberto,
                    // Sempre será usado apenas uma vez
                    //  R0|CIOT|CPF|CNPJ| implementar interface
                    $R0_infCIOT= $dom->createElement("infCIOT");
                    $R0_CIOT =  $dom->createElement("CIOT",$dados[1]);
                    $R0_infCIOT->appendChild($R0_CIOT);

                    $R0_CPF =  $dom->createElement("CPF",$dados[2]);
                    $R0_infCIOT->appendChild($R0_CPF);

                    $R0_CNPJ =  $dom->createElement("CNPJ",$dados[3]);
                    $R0_infCIOT->appendChild($R0_CNPJ);

                    //print_r($dom->getElementsByTagName("RNTRC"));exit();

                    $RNTRC->insertBefore($R0_infCIOT);
                    break;
                case "R1":
				...
				case "G11":
                    ...
                    $infUnidTransp->appendChild($idUnidTransp);
						// G11 - G111 - G112 - G1121
					$qtdRat = $dom->createElement("qtdRat", $dados[3]);
				...
				case "G1121":
                    ...
                    break;
                case "H":
                    // H|respSeg|CNPJ|CPF|xSeg|CNPJ|nApol|nAver|
                    $H_seg = $dom->createElement("seg");

                    $H_infResp = $dom->createAfter($seg,"infResp");
                    $H_respSeg = $dom->createElement("respSeg",$dados[1]);
                    $H_infResp->appendChild($H_respSeg);
                    
                    $H_CNPJ = $dom->createElement("CNPJ",$dados[2]);
                    $H_infResp->appendChild($H_CNPJ);

                    $H_CPF = $dom->createElement("CPF",$dados[3]);
                    $H_infResp->appendChild($H_CPF);

                    $H_infSeg = $dom->createAfter($seg,"infSeg");
                    $H_xSeg = $dom->createElement("xSeg",$dados[4]);
                    $H_infSeg->appendChild($H_xSeg);
                    
                    $H_CNPJ_SEG = $dom->createElement("CNPJ",$dados[5]);
                    $H_infSeg->appendChild($H_CNPJ_SEG);
                    
                    $H_apolice = $dom->createElement("nApol",$dados[6]);
                    $H_infSeg->appendChild($H_apolice);
                    
                    $H_averbacao = $dom->createElement("nAver",$dados[7]);
                    $H_infSeg->appendChild($H_averbacao);
     ...
     * @param object $dom 
     */
    private function montaChaveXML($dom,$versao)
    {
