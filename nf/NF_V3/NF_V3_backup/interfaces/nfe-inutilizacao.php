<?php 
    // cat /user/nfe/26361804000290/CaixaEntrada/Processado/NFE-000000286-NE-000290-20190627-161748.TXT
    
    // php /var/www/html/nf/NF_V3/NF_V3/interfaces/nfe-inutilizacao.php 26361804000290 /user/nfe/26361804000290/CaixaSaida/Sefaz/INFER-2019.06.28_16.07.47.txt 19   1  286       286 'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX' /user/nfe/26361804000290/RET/2019/06/28/ 2 SVRS
    ...
    */
    ...
    if (!isset($argv[1])){
        echo "
            ENTRADA:
                [ 1] = cnpj              =  argv[ 1]            ; 
                [ 2] = arquivo_txt_saida =  argv[ 2]            ; 
                [ 3] = ano               =  argv[ 3]            ; 
                [ 4] = serie             =  argv[ 4]            ; 
                [ 5] = nota_inicial      =  argv[ 5]            ; 
                [ 6] = nota_final        =  argv[ 6]            ; 
                [ 7] = justificativa     =  argv[ 7]            ; 
                [ 8] = caminho_contabil  =  argv[ 8]            ; 
                [ 9]*= ambiente_numerico =  argv[ 9] |      '2' ; 
                [10]*= estado            =  argv[10] |     'PR' ; 
                [11]*= autorizadora      =  argv[11] | argv[10] ; 
            
            SAIDA:
                cStat       | WE-RET-STATUS       ; 
                xMotivo     | WE-RET-DESCRICAO    ; 
                nProt       | WE-RET-STATUS       ; 
                xml_retorno | WE-RET-XML-INTEGRAL ; 
    ...
    $estado = (isset($argv[10])?$argv[10]:'PR'); // Se não for passado o parametro, buscar estado RS
    ...
    */
    ...
    if ($arquivo_txt_saida == 'debug'){
        print(formatXML($xmlFinal));
    }
    
    $xml_retorno = $soap->comunicar($arquivo_txt_saida == 'debug');
    if ($arquivo_txt_saida == 'debug'){
        print(formatXML($xml_retorno));
    }
