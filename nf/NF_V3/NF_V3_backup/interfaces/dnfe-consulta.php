<?php 
    ...
    */ 
    ...
    $template_soap = preg_replace("/\s+/"       ,  ' '  , $template_soap);
    $template_soap = preg_replace("/\>\s*\</"   ,  '><' , $template_soap);
    $template_soap = preg_replace("/^\s+|\s+$/" ,  ''   , $template_soap);
    /**
     * CTE FIM
     */
    ...
    $xml_webservice = $soap->comunicar(true);
    
    echo formatXML($xml_webservice);
    
    // echo preg_replace("/[\w\W]*\<cStat\>(\d*)\<\/cStat\>[\w\W]*/", '$1', $xml_webservice);
    
    httpStatus( preg_replace("/[\w\W]*\<cStat\>(\d*)\<\/cStat\>[\w\W]*/", '$1', $xml_webservice) );
    
    exit();
    
    $time = time();
    file_put_contents("$temp$cnpj-$time", $xml_webservice);
