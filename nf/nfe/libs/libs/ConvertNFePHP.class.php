...
class ConvertNFePHP
{
        ...
        for ($l = 0; $l < count($arquivo); $l++) {
            ...
            switch ($dados[0]) {
                ...
                case "B": //identificadores [infNFe]
					...
					// Obtem a contingencia atraves do cadastro do contribuinte
					...
					if(!empty($this->arrayContribuinte['justificativa_contingencia'])){
                    //if (!empty($dados[20]) || !empty($dados[21])) {
                        $dhCont = $dom->createElement("dhCont", date("Y-m-d\TH:i:s", strtotime($this->arrayContribuinte['data_hora_contingencia'])));
                        $ide->appendChild($dhCont);
                        $xJust = $dom->createElement("xJust", $this->arrayContribuinte['justificativa_contingencia']);
                ...
                case "E":
                    ...
                    if ($this->tpAmb == '2') {
                        ...
                        $xNome = $dom->createElement("xNome", $dados[1]);
                        $dest->appendChild($xNome);
						if(empty($dados[2])){ $dados[2] = "ISENTO"; } // guilherme: alterado para validar com XSD
                ...
                case "G":
                    ...
                    //G|CNPJ CPF|xLgr|nro|xCpl|xBairro|cMun|xMun|UF|
                    $entrega = $dom->createElement("entrega");
					switch(strlen($dados[1])){
						case 11:
							$entregaCPF = $dom->createElement("CPF", $dados[1]);
							$entrega->appendChild($entregaCPF);
						break;
						default:
							$entregaCNPJ = $dom->createElement("CNPJ", $dados[1]);
							$entrega->appendChild($entregaCNPJ);
						break;
					}
                    if (!empty($dados[2])) {
                ...
                case "I25":
                    ...
                    if (!empty($dados[2])) {
                        $nSeqAdic = $dom->createElement("nSeqAdic", ltrim($dados[2],0));
                ...
                case "L":
                    ...
                    break;
                //case "L101": Substituido para no inventti não dar erro.
				case "L01":
    ...
    private function montaChaveXML($dom)
    {
		...
		/* Em caso de não informado o número da nota fiscal (será assumido um valor randomico) */
        if (strlen($cNF) != 8) {
