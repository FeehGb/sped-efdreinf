...
class ConvertNFePHP
{
	...
	/**
     * MensagemErro
     * Descritivo da Mensagem de Erro
     * @var string
     */
    public $mensagemErro = "";
	...
	/**
     * Email Destinatário
     * Email do Destinatário que não pode ser incorporado no XML, pois é passível de erro
     * @var string
     */
    ...
    protected function nfeTxt2XmlArrayComLinhas($arrayComAsLinhasDoArquivo)
    {
        ...
        for ($l = 0; $l < count($arquivo); $l++) {
			...
			$this->limparString = false;
			
			if($dados[0] == "Z" || $dados[0] == "Z04"){
				if (!empty($dados[1])) {
					$dados[1] = trim(preg_replace('/\s\s+/', " ", $dados[1]));
					if ($this->limparString) {
						$dados[1] = $this->limpaString($dados[1]);
					}
				} //end if
				if (!empty($dados[2])) {
					$this->informacaoAdicional = $dados[2] = trim($dados[2]);
				} //end if
			}else{
				for ($x = 0; $x < count($dados); $x++) {
					if (!empty($dados[$x])) {
						$dados[$x] = str_replace("&","E",$dados[$x]);
            ...
            switch ($dados[0]) {
                ...
                case "B": //identificadores [infNFe]
                    ...
                    $this->modelo = $dados[5];
                    $ide->appendChild($mod);
                    $serie = $dom->createElement("serie", ltrim($dados[6],"0")); // guilherme: adicionado ltrim 0
                    $ide->appendChild($serie);
                    $nNF = $dom->createElement("nNF", ltrim($dados[7],"0")); // guilherme: adicionado ltrim 0
                    $ide->appendChild($nNF);
					...
					// Volta a fazer o tpEmis original
					$this->tpEmis = $dados[14];
                ...
                case "B03":
                    ...
                    //B03|cUF|AAMM(ano mês)|CNPJ|Mod|serie|nNF|
                    if (!isset($NFref)) {
                        $notas[$currnota]['NFref'] = $dom->createElement("NFref");
                        $NFref = & $notas[$currnota]['NFref'];
                        $ide->appendChild($NFref);
                    ...
                    $serie = $dom->createElement("serie", ltrim($dados[5],"0")); // guilherme: alterado para validar com XSD
                    $refNF->appendChild($serie);
                    $nNF = $dom->createElement("nNF", ltrim($dados[6],"0")); // guilherme: alterado para validar com XSD
                ...
                case "BA10":
                    ...
                    $refNFP->appendChild($AAMM);
					          if(empty($dados[3])){ $dados[3] = "ISENTO"; }
                ...
                case "C":
					...
					if(empty($dados[3])){ $dados[3] = "ISENTO"; } // guilherme: alterado para validar com XSD
                    $IE = $dom->createElement("IE", $dados[3]);
                    $emit->appendChild($IE);
                    if (!empty($dados[4])) {
                ...
                case "C05":
                    ...
                    $nro = $dom->createElement("nro", str_pad($dados[2], 5, "0", STR_PAD_LEFT));
                ...
                case "E":
                    ...
                    if ($this->tpAmb == '2') {
                        ...
                        $xNome = $dom->createElement("xNome", $dados[1]);
                        $dest->appendChild($xNome);
            						if(empty($dados[2])){ $dados[2] = "2"; } // guilherme: alterado para validar com XSD
            						$indIEDest = $dom->createElement("indIEDest", $dados[2]);
                                    $dest->appendChild($indIEDest);
            						if (!empty($dados[3])) {
            							$IE = $dom->createElement("IE", $dados[3]);
                ...
                case "E02":
                    ...
                    if ($this->tpAmb == '2') {
                        if ($dados[1] != '') {
                            ...
                            //$this->CNPJdest = '99999999000191';
                            $CNPJ = $dom->createElement("CNPJ", $dados[1]);
                            $this->CNPJdest = $dados[1];
                        } else {
                ...
                case "E03":
                    ...
                    //se ambiente homologação preencher conforme NT2011.002,
                    //válida a partir de 01/05/2011
         /*roy_madeiramadeira if ($this->tpAmb == '2') {
                        if ($dados[1] != '') {
                            //operação nacional em ambiente homologação usar 99999999000191
                            $CNPJ = $dom->createElement("CNPJ", '99999999000191');
                            $this->CNPJdest = '99999999000191';
                        ...
                        $dest->insertBefore($dest->appendChild($CNPJ), $xNome);
                    } else { roy_madeiramadeira*/
                        $CPF = $dom->createElement("CPF", $dados[1]);
                        $this->CNPJdest = $dados[1];
                        $dest->insertBefore($dest->appendChild($CPF), $xNome);
                   //roy_madeiramadeira } //fim teste ambiente
                    break;
                case "E03a":
                    //idEstrangeiro [dest]
                ...
                case "G":
                    ...
                    $entrega = $dom->createElement("entrega");
          					$dados1 = trim($dados[1],"0");
          					if (!empty($dados1)) {
          						if(strlen($dados[1]) == 11){
          							$CPF = $dom->createElement("CPF", $dados[1]);
          							$entrega->appendChild($CPF);
          						}elseif(strlen($dados[1]) == 14){
          							$CNPJ = $dom->createElement("CNPJ", $dados[1]);
                ...
                case "H":
                    ...
                    $det->setAttribute("nItem", ltrim($dados[1],"0")); // gjps adicionado ltrim 0
                    if (!empty($dados[2])) {
                ...
                case "I":
                    //PRODUTO SERVICO [det]
                    //I|cProd|cEAN|xProd|NCM|EXTIPI|CEST|CFOP|uCom|qCom|vUnCom|vProd|cEANTrib|uTrib|qTrib|vUnTrib
                    ...
                    if($this->tpAmb == 2 && $this->modelo == "65"){
                        $dados[3] = "NOTA FISCAL EMITIDA EM AMBIENTE DE HOMOLOGACAO - SEM VALOR FISCAL";
                    }
                    $xProd = $dom->createElement("xProd", $dados[3]);
                    $prod->appendChild($xProd);
                    $NCM = $dom->createElement("NCM", $dados[4]);
                    $prod->appendChild($NCM);
                    $CEST = ltrim($dados[24],"0");
                    if (!empty($CEST)) { // Alteração nota tecnica 2015.003 ICMS interestadual
                        $CEST = $dom->createElement("CEST", $dados[24]);
                    ...
                    if (!empty($dados[12])) {
                        ...
                        //$uTrib = $dom->createElement("uTrib", str_pad($dados[7],3,".",STR_PAD_RIGHT));
                    }
                    ...
                    if (!empty($dados[15]) && $dados[15] != "0.00") { // guilherme: alterado para funcionar na validacao do XSD
                        $vFrete = $dom->createElement("vFrete", $dados[15]);
                        $prod->appendChild($vFrete);
                    }
                    if (!empty($dados[16]) && $dados[16] != "0.00") { // guilherme: alterado para funcionar na validacao do XSD
						        $vSeg = $dom->createElement("vSeg", $dados[16]);
                        $prod->appendChild($vSeg);
                    }
                    if (!empty($dados[17]) && $dados[17] != "0.00") { // guilherme: alterado para funcionar na validacao do XSD
                        $vDesc = $dom->createElement("vDesc", $dados[17]);
                        $prod->appendChild($vDesc);
                    }
                    if (!empty($dados[18]) && $dados[18] != "0.00") { // guilherme: alterado para funcionar na validacao do XSD
                    ...
                    if (!empty($dados[23])) {
                            $nRECOPI = $dom->createElement("nRECOPI", $dados[23]);
                            $prod->appendChild($nRECOPI);
                        }
				 ...
				 case "I05a":
                    ...
                    //I05a|nDI|dDI|xLocDesemb|UFDesemb|dDesemb|tpViaTransp|vAFRMM|tpIntermedio|CNPJ|UFTerceiro|cExportador|
					if (!empty($dados[1])) {
						...
						$prod->insertBefore($det->appendChild($NVE), $EXTIPI);
                    }
                case "I18":
                    //Tag da Declaração de Importação [prod]
                    //I18|nDI|dDI|xLocDesemb|UFDesemb|dDesemb|cExportador|tpViaTransp|vAFRMM|tpIntermedio|CNPJ|UFTerceiro|
                    ...
                    if (!empty($dados[7])) {
                        $tpViaTransp = $dom->createElement("tpViaTransp", ltrim($dados[7],"0"));
                        $DI->appendChild($tpViaTransp);
                    }
                    if (!empty($dados[8])) {
                        $vAFRMM = $dom->createElement("vAFRMM", $dados[8]);
                        $DI->appendChild($vAFRMM);
                    }
                    if (!empty($dados[9])) {
                        $tpIntermedio = $dom->createElement("tpIntermedio", $dados[9]);
                        $DI->appendChild($tpIntermedio);
                    }
					$dados10 = trim($dados[10],"0");
                    if ((empty($dados[10]) == false) && (empty($dados10) == false)) {
                        $CNPJ = $dom->createElement("CNPJ", $dados[10]);
                        $DI->appendChild($CNPJ);
                    }
                    if (!empty($dados[11])) {
                        $UFTerceiro = $dom->createElement("UFTerceiro", $dados[11]);
                ...
                case "I25":	
                    ...
                    if (!empty($dados[1])) {
                        $nAdicao = $dom->createElement("nAdicao", ltrim($dados[1],0));
                        $adi->appendChild($nAdicao);
                    }
                    if (!empty($dados[2])) {
                        $nSeqAdicC = $dom->createElement("nSeqAdic", ltrim($dados[2],0));
                    ...
                    if (!empty($dados[4]) && $dados[4] != "0.00") { // guilherme: alterado para funcionar na validacao do XSD
                ...
                case "I50":
					...
					if (!isset($xPed) && !isset($nItemPed)) {
                        ...
                        if (!isset($xPed)) {
                            $prod->insertBefore($prod->appendChild($detExport), $nItemPed);
                        } else {
                            $prod->insertBefore($prod->appendChild($detExport), $xPed);
                        }
                    }
                    break;
                case "I52":
				...
				case "L01":
					...
					//INVENTTI: L01|cProdANP|CODIF|qTemp|UFCons|pMixGN|
                    $comb = $dom->createElement("comb");
                    $cProdANP = $dom->createElement("cProdANP", $dados[1]);
                    $comb->appendChild($cProdANP);
					// Alterado para adequar-se com a Inventti
                    if (!empty($dados[5]) && ($dados[1] == "210203001")) {
                        $pMixGN = $dom->createElement("pMixGN", $dados[5]);
                ...
                case "M":
                    ...
                    if (strlen($vTotTrib)>0) {
						if ($vTotTrib != "0.00") { // guilherme: alterado para funcionar na validacao do XSD
                ...
                case "N08":
					...
					if(empty($dados[3])){ $dados[3] = "0"; } // guilherme: alterado para XSD
                ...
                case "N10":
					...
					$dados15 = trim($dados[15],"0");
                    if (!empty($dados[14]) || !empty($dados15)) {
                        $vICMSDeson = $dom->createElement("vICMSDeson",$dados[14]);
                ...
                case "N10a":
                    ...
                    if (!empty($dados[6])) {
                        $pRedBC = $dom->createElement("pRedBC", $dados[6]);
                ...
                case "N10h":
                    ...
                    break;
                case "NA01": // NT 2015.003 - ICMS Interestadual
                    //NA01|vBCUFDest |pFCPUFDest |pICMSUFDest |pICMSInter |pICMSInterPart |vFCPUFDest
                    //|vICMSUFDest |vICMSUFRemet 
                    $auxZerado = 0;
                    $ICMSUFDest  = $dom->createElement("ICMSUFDest");
                    
                    $vBCUFDest = trim($dados[1],"0");
                    //jmh1 if($vBCUFDest != "" && $vBCUFDest != "."){
                        $vBCUFDest = $dom->createElement("vBCUFDest", $dados[1]);
                        $ICMSUFDest->appendChild($vBCUFDest);
                        $auxZerado++;
                    //jmh1 }
                    
                    $pFCPUFDest = trim($dados[2],"0");
                    //jmh1 if($pFCPUFDest != "" && $pFCPUFDest != "."){
                        $pFCPUFDest  = $dom->createElement("pFCPUFDest", $dados[2]);
                        $ICMSUFDest->appendChild($pFCPUFDest);
                        $auxZerado++;
                    //jmh1 }
                        
                    $pICMSUFDest = trim($dados[3],"0");
                    //jmh1 if($pICMSUFDest != "" && $pICMSUFDest != "."){
                        $pICMSUFDest = $dom->createElement("pICMSUFDest", $dados[3]);
                        $ICMSUFDest->appendChild($pICMSUFDest);
                        $auxZerado++;
                    //jmh1 }
                        
                    $pICMSInter = trim($dados[4],"0");
                    //jmh1 if($pICMSInter != "" && $pICMSInter != "."){
                        $pICMSInter = $dom->createElement("pICMSInter", $dados[4]);
                        $ICMSUFDest->appendChild($pICMSInter);
                        $auxZerado++;
                    //jmh1 }
                    
                    $pICMSInterPart = trim($dados[5],"0");
                    //jmh1 if($pICMSInterPart != "" && $pICMSInterPart != "."){
                        $pICMSInterPart = $dom->createElement("pICMSInterPart", $dados[5]);
                        $ICMSUFDest->appendChild($pICMSInterPart);
                        $auxZerado++;
                    //jmh1 }
                    
                    $vFCPUFDest = trim($dados[6],"0");
                    //jmh1 if($vFCPUFDest != "" && $vFCPUFDest != "."){
                        $vFCPUFDest = $dom->createElement("vFCPUFDest", $dados[6]);
                        $ICMSUFDest->appendChild($vFCPUFDest);
                        $auxZerado++;
                    //jmh1 }
                    
                    $vICMSUFDest = trim($dados[7],"0");
                    //jmh1 if($vICMSUFDest != "" && $vICMSUFDest != "."){
                        $vICMSUFDest  = $dom->createElement("vICMSUFDest", $dados[7]);
                        $ICMSUFDest->appendChild($vICMSUFDest);
                    //jmh1 }
                    
                    $vICMSUFRemet = trim($dados[8],"0");
                    //jmh1 if($vICMSUFRemet != "" && $vICMSUFRemet != "."){
                        $vICMSUFRemet = $dom->createElement("vICMSUFRemet", $dados[8]);
                ...
                case "Q03":
                    ...
                    $qBCProd = $dom->createElemento("qBCProd", $dados[2]);
                ...
                case "U":
					...
					$indISS = $dom->createElement("indISS", $dados[11]); // guilherme add
                    $ISSQN->appendChild($indISS);
					$cServico = $dom->createElement("cServico", $dados[12]); // guilherme add
                    $ISSQN->appendChild($cServico);
					$cMun = $dom->createElement("cMun", $dados[13]); // guilherme add
                    $ISSQN->appendChild($cMun);
					$cPais = $dom->createElement("cPais", $dados[14]); // guilherme add
                    $ISSQN->appendChild($cPais);
					$nProcesso = $dom->createElement("nProcesso", $dados[15]); // guilherme add
                    $ISSQN->appendChild($nProcesso);
					$indIncentivo = $dom->createElement("indIncentivo", $dados[16]); // guilherme add
                ...
                case "W02":
                    ...
                    /* Alteração NT 2015.003 ICMS Interestadual */
                    $vFCPUFDest = ltrim($dados[17],"0");
                    //if($vFCPUFDest != ""){
                        $vFCPUFDest  = $dom->createElement("vFCPUFDest", $dados[17]);
                        $ICMSTot->appendChild($vFCPUFDest);
                    //}
                    $vICMSUFDest = ltrim($dados[18],"0");
                    //if($vICMSUFDest != ""){
                        $vICMSUFDest  = $dom->createElement("vICMSUFDest", $dados[18]);
                        $ICMSTot->appendChild($vICMSUFDest);
                    //}
                    $vICMSUFRemet = ltrim($dados[19],"0");
                    //if($vICMSUFRemet != ""){
                        $vICMSUFRemet = $dom->createElement("vICMSUFRemet", $dados[19]);
                ...
                case "X03":
					...
					// Alterado pois a inventti não quis seguir o padrão do SEFAZ e temos que nos adaptar a eles
                    if (!empty($dados[5])) {
                ...
                case "Y02":
                    ...
                    if (!empty($dados[3])) {
						if ($dados[3] != "0.00") { // guilherme: alterado para funcionar na validacao do XSD
                ...
                case "YA":
                  ...
                  if(!empty($dados[1])){
                          ...
                          $tpIntegra = $dom->createElement("tpIntegra",$dados[1]);
                          $card->appendChild($tpIntegra);
                          $CNPJ = $dom->createElement("CNPJ",$dados[2]);
                          $card->appendChild($CNPJ);
                          $tBand = $dom->createElement("tBand",$dados[3]);
                          $card->appendChild($tBand);
                          $cAut = $dom->createElement("cAut",$dados[4]);
                ...
                case "YA09":
                  if(!empty($dados[1])){
                          $vTroco = $dom->createElement("vTroco",$dados[1]);
                          $pag->appendChild($vTroco);
                        }
                  break;
                ...
                case "Z04":
                    ...
                    $obsCont->setAttribute("xCampo", trim(str_replace("!","",$dados[1])));
                    $xTexto = $dom->createElement("xTexto", trim(str_replace("!","",$dados[2])));
                ...
                case "ZX02":
                    ...
                    //ZX02|IDToken|Token
                    //todos os campos são obrigatorios
                    $this->idToken = $dados[1];
                    $this->token = $dados[2];
                    break;
                case "@":
                    $this->emailDestinatario = "";
                    $localArrayEmail;
                    if(trim($dados[1]) != "" && $dados[1] != '0'){
                        $localArrayEmail[] = $dados[1];
                    }
                    if(trim($dados[2]) != "" && $dados[2] != '0'){
			$localArrayEmail[] = $dados[2];
                    }
                    if(is_array($localArrayEmail)){
                        $this->emailDestinatario = implode(";",$localArrayEmail);
                    }
                    break;
            } //end switch
        } //end for
     ...
     * Monta a chave da NFe de 44 digitos com base no XML via string
     * Isso é útil no caso da chave formada no txt estar errada
     * 
     * @name montaChaveXML
     * @param object $dom 
    ...
    public function montaChaveXMLExterno($pXmlString=""){
		...
		//$dom = simplexml_load_string($pXmlString);
    ...
    /**
     * incluirTagNFCe
     * incluir ta g infNFeSupl na nota fiscal gerada
     * 
     * @name incluirTagNFCe
     * @param object $dom 
     */
    public function incluirTagNFCe($pUrl,$pXml,$pApenasGerar="N"){
        //ler e editar xml
        ...
        // e as tags vazias
        $xmldoc->loadXML($pXml,LIBXML_NOBLANKS | LIBXML_NOEMPTYTAG);
        $digestValue = $xmldoc->getElementsByTagName("DigestValue")->item(0)->nodeValue;
        $NFe = $xmldoc->getElementsByTagName("NFe")->item(0);
        $Signature = $xmldoc->getElementsByTagName("Signature")->item(0);
        $infNFeSupl = $xmldoc->createElement("infNFeSupl");
        //$NFe->appendChild($infNFeSupl);
        $NFe->insertBefore($infNFeSupl,$Signature);
        if($this->CNPJdest == ""){
            $qrCode = "chNFe=".$this->chave."&nVersao=100&tpAmb=".$this->tpAmb."&dhEmi=".$this->String2Hex($this->dhEmis)."&vNF=".$this->vNF."&vICMS=".$this->vICMS."&digVal=".$this->String2Hex($digestValue)."&cIdToken=".str_pad($this->idToken,6,"0", STR_PAD_LEFT).$this->token;
            $cHashQRCode = sha1($qrCode);
            $qrCode = $pUrl."?chNFe=".$this->chave."&nVersao=100&tpAmb=".$this->tpAmb."&dhEmi=".$this->String2Hex($this->dhEmis)."&vNF=".$this->vNF."&vICMS=".$this->vICMS."&digVal=".$this->String2Hex($digestValue)."&cIdToken=".str_pad($this->idToken,6,"0", STR_PAD_LEFT)."&cHashQRCode=".strtoupper($cHashQRCode);
        }else{
            $qrCode = "chNFe=".$this->chave."&nVersao=100&tpAmb=".$this->tpAmb."&cDest=".$this->CNPJdest."&dhEmi=".$this->String2Hex($this->dhEmis)."&vNF=".$this->vNF."&vICMS=".$this->vICMS."&digVal=".$this->String2Hex($digestValue)."&cIdToken=".str_pad($this->idToken,6,"0", STR_PAD_LEFT).$this->token;
            $cHashQRCode = sha1($qrCode);
            $qrCode = $pUrl."?chNFe=".$this->chave."&nVersao=100&tpAmb=".$this->tpAmb."&cDest=".$this->CNPJdest."&dhEmi=".$this->String2Hex($this->dhEmis)."&vNF=".$this->vNF."&vICMS=".$this->vICMS."&digVal=".$this->String2Hex($digestValue)."&cIdToken=".str_pad($this->idToken,6,"0", STR_PAD_LEFT)."&cHashQRCode=".strtoupper($cHashQRCode);        
        }
        $this->qrCode = $qrCode;
        $stringQrCode = $xmldoc->createCDATASection($qrCode);
        /* Quando o SEFAZ ainda não aceitar a tag infNFeSupl retornar em memória apenas para o Cobol montar posteriormente a DANFE */
            if($pApenasGerar == "S"){
                return $pXml;
            }
        $qrCode = $xmldoc->createElement("qrCode");
