...
//                        $nome = $doc->save($nome);
    ...
    public function getProtocol3($recibo = '', $chave = '', $tpAmb = '', &$aRetorno = array(), $arquivoRetorno = '', $cnpj = '', $tipoEmissao = '')
    {
        try {
            ...
            $conteudoRetorno = $cnpj."|".$this->pSimpleGetValue($doc, 'cUF')."|".$tipoEmissao."|".$this->pSimpleGetValue($doc, 'tpAmb')."|".$this->pSimpleGetValue($doc, 'chNFe')."|".$this->pSimpleGetValue($doc, 'cStat')."|".$this->pSimpleGetValue($doc, 'xMotivo')."|||||".$aRetorno['xmlRetorno']."|";
...
/*            if (!file_put_contents($arqName, $procXML)) {
     ...
     */
    public function envCCe($dhEvento_cobol='', $chNFe='',$xCorrecao='',$nSeqEvento='1', $pLote='', $tpAmb='',$modSOAP='2'){
        try {
            ...
            $dhEvento = substr($dhEvento_cobol,0,4)."-".substr($dhEvento_cobol,4,2)."-".substr($dhEvento_cobol,6,2).'T';
            $dhEvento .= substr($dhEvento_cobol,8,2).":".substr($dhEvento_cobol,10,2).":".substr($dhEvento_cobol,12,2).$this->timeZone;
...
//          print_r($retorno);
			...
			$this->arrayRetorno['xml_envio'] = $dados;
    ...
    */
    public function __validCerts($cert='',&$aRetorno=''){
        try{
            ...
            $aRetorno = array('status'=>$flagOK,'error'=>$errorMsg,'meses'=>$monthsToExpire,'dias'=>$daysToExpire,'day'=>$dia,'month'=>$mes,'year'=>$ano);
        } catch (nfephpException $e) {
            $this->__setError($e->getMessage());
            if ($this->exceptions) {
                throw $e;
