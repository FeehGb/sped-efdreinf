...
/**
     ...
     */
    function __construct($pCnpj='', $pAmbiente=0, $pGrupo='', $mododebug=0, $exceptions=false, $pScan=false, $pAn=false){
		...
		*/
		...
		if($pScan != ""){
			...
			$this->aURL = $this->loadSEFAZ("/var/www/html/nf/nfe/novo/config/nfe_ws3_mod55.xml",$this->tpAmb,$pScan);
		}else{
			//$this->aURL = $this->loadWebServices($this->tpAmb,$this->UF);
			$this->aURL = $this->loadSEFAZ("/var/www/html/nf/nfe/novo/config/nfe_ws3_mod55.xml",$this->tpAmb,$this->UF);
		}
    ...
    */
    ...
    public function autoriza($sxml, $idLote, &$aRetorno = array(), $indSinc = 0)
    {
        try {
            ...
            //verifica o retorno
//file_put_contents("/user/nfe/logRetorno",date("d/m/Y H:i:s")." ----- ".$retorno."\n\n",FILE_APPEND); // Log de confirmacao
            if (!$retorno) {
				$msg = "Nao houve retorno Soap verifique a mensagem de erro e o debug!!";
				$this->__setError($msg);
                throw new nfephpException($msg);
...
//                        $nome = $doc->save($nome);
    ...
    public function getProtocol3($recibo = '', $chave = '', $tpAmb = '', &$aRetorno = array())
    {
        try {
            ...
            //envia a solicitação via SOAP
            if (!$retorno = $this->__sendSOAP2($urlservico, $namespace, $cabec, $dados, $metodo, $tpAmb)) {
    ...
    public function getListNFe($AN=true,$indNFe='0',$indEmi='0',$ultNSU='',$tpAmb='',$modSOAP='2',&$resp=''){ 
        try {
            ...
            if (!$retorno){
                //não houve retorno
                $msg = "Nao houve retorno da SEFAZ, tente novamente mais tarde!";
                throw new nfephpException($msg);
            }
        ...
        $resp = array('indCont'=>$indCont,'nsu'=>$ultNSU,'NFe'=>$aNFe,'Canc'=>$aCanc,'CCe'=>$aCCe, 'retorno'=>$retorno);
        return $retorno;
    }//fim getListNFe
...
/*            if (!file_put_contents($arqName, $procXML)) {
    ...
    public function manifDest($chNFe='',$tpEvento='',$xJust='',$tpAmb='',$modSOAP='2',&$resp=''){
        try {
            ...
            /*
            @file_put_contents("/var/www/html/eduardo/urlservico_gui.txt", $urlservico);
            @file_put_contents("/var/www/html/eduardo/namespace_gui.txt", $namespace);
            @file_put_contents("/var/www/html/eduardo/cabec_gui.txt", $cabec);
            @file_put_contents("/var/www/html/eduardo/dados_gui.txt", $dados);
            @file_put_contents("/var/www/html/eduardo/metodo_gui.txt", $metodo);
            @file_put_contents("/var/www/html/eduardo/this_tpAmb_gui.txt", $this->tpAmb);
            */
            ...
            $resp = array('bStat'=>true,'cStat'=>$cStat,'xMotivo'=>$xMotivo,'nProt'=>$nProt,'dhRegEvento'=>$dhRegEvento,'arquivo'=>$filename);
            //salva o arquivo xml
			...
			$resp = array('bStat'=>false,'cStat'=>$cStat,'xMotivo'=>$xMotivo,'nProt'=>'','dhRegEvento'=>$dhRegEvento,'arquivo'=>'');
            return false;
        }    
        return $retorno;
    } //fim manifDest
...
file_put_contents("/user/nfe/xml.log2",$data."\n\n".$__xml."\n\n",FILE_APPEND);
