...
class MDFeNFePHP {
    ...
    **/
     ...
     */
    public function mdfeConsNaoEnc($CNPJ='', $UF = '', $tpAmb='', $modSOAP='2',&$resp=''){
 // Retorno da funçao
        $aRetorno = array('bStat' => false,'cStat' => '','tMed'  => '','dhRecbto' => '','xMotivo' => '','xObs' => '');
        // Caso o parametro tpAmb seja vazio
        if ($tpAmb == '') {
        ...
        $cUF = $this->cUFlist[$UF];
        // Identificação do serviço
        $servico = 'MDFeConsultaNaoEncerrados';
        // Recuperação da versão
        $versao = $aURL[$servico]['version'];
        // Recuperação da url do serviço
        $urlservico = $aURL[$servico]['URL'];
		...
		echo $retorno;
		file_put_contents('/var/www/html/nf/nfe/novo/temp/'.$CNPJ.date('H-i-s').".txt");
        // Verifica o retorno do SOAP
        /*if (isset($retorno)) {
            // Tratar dados de retorno
            $doc = new DOMDocument();
