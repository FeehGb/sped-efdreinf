 ...
 * @description tion Programa elaborado para efetuar comunica��o do HTML com as classes control.
 * @TODO 		Fazer tudo
*/
	
	require_once("/var/www/html/nf/nfe/novo/control/CContribuinte.php");
	require_once("/var/www/html/nf/nfe/novo/control/CWebService.php");
	require_once("/var/www/html/nf/nfe/novo/control/CInutilizar.php");
	require_once("/var/www/html/nf/nfe/novo/control/CCancelar.php");
	require_once("/var/www/html/nf/nfe/novo/control/CNotaFiscal.php");
	require_once("/var/www/html/nf/nfe/novo/control/CCartaCorrecao.php");
	...
	if(substr($funcao,0,4) == "NFE-"){
		...
		fCC($funcao);
	}elseif(substr($funcao,0,6)  == "DANFE-"){
		fDanfe($funcao);
	...
	// Tratar as fun��es que s�o provenientes das Notas Fiscais
	
	function fNfe($pFuncao){
		global $filialGrupo;
		// Variareis locais
		$CNf = new CNotaFiscal($filialGrupo);
		
		$retornoJson;
		$periodo_ini	= substr($_POST['tConsultaPeriodoInicial'],6,4)."-".substr($_POST['tConsultaPeriodoInicial'],3,2)."-".substr($_POST['tConsultaPeriodoInicial'],0,2);
		$periodo_fim	= substr($_POST['tConsultaPeriodoFinal'],6,4)."-".substr($_POST['tConsultaPeriodoFinal'],3,2)."-".substr($_POST['tConsultaPeriodoFinal'],0,2);

		if(substr($pFuncao,0,10) == "NFE-LISTAR"){
			/*$cliente = trim($_POST['tConsultaPeriodoInicial']) != "" ? trim($_POST['tConsultaPeriodoInicial']) : "";
			if($cliente != ""){
				$CNf->nome_destinatario = "%".str_replace(" ","%",$cliente)."%";
			}
			/*$transportador = trim($_POST['tConsultaTransportadora']) != "" ? trim($_POST['tConsultaTransportadora']) : "";
			$ufDestino = trim($_POST['sConsultaUFDestino']) != "" ? trim($_POST['sConsultaUFDestino']) : "";
			$cfop = trim($_POST['tConsultaCFOP']) != "" ? trim($_POST['tConsultaCFOP']) : "";*/
		}
		switch($pFuncao){
			case "NFE-LISTAR-CONTRIBUINTES":
				...
				exit();
			break;
			case "NFE-LISTAR-PENDENTES":
				$contribuinte = explode("-",$_POST['sContribuinte']);
				if($contribuinte[0] == "consolidado"){
					$CNf->cnpj_emitente	= "";	
				}else{
					$CNf->cnpj_emitente	= $contribuinte[0];	
				}
				$CNf->ambiente 		= $contribuinte[1];
				$CNf->status 		= array("01","02");
				$CNf->periodo_ini	= $periodo_ini;
				$CNf->periodo_fim	= $periodo_fim;
	
				$return = $CNf->mObterListaTela();
				...
				exit();
			break;
			case "NFE-LISTAR-AUTORIZADAS":
				$contribuinte = explode("-",$_POST['sContribuinte']);
				if(is_numeric($contribuinte[0])){
					$CNf->cnpj_emitente	= $contribuinte[0];
				}
				$CNf->ambiente 		= $contribuinte[1];
				$CNf->status 		= "03";
			...
			case "NFE-LISTAR-CANCELADAS":
				...
				exit();
			break;
			case "NFE-LISTAR-INCONSISTENTES":
				$contribuinte = explode("-",$_POST['sContribuinte']);
				$CNf->cnpj_emitente	= $contribuinte[0];
				$CNf->ambiente 		= $contribuinte[1];
				$CNf->status 		= array("04","05");
				$CNf->periodo_ini	= $periodo_ini;
				$CNf->periodo_fim	= $periodo_fim;
			...
			case "NFE-ATUALIZAR-SEFAZ":
				$CNf->cnpj_emitente	= $_POST['hCnpj'];
				$CNf->ambiente 		= $_POST['hAmbiente'];
				$CNf->numero_nota 	= $_POST['hAtualizaNF'];
				$CNf->serie_nota 	= $_POST['hAtualizaSerie'];

				$return = $CNf->mAtualizarNotaSefaz();
			...
			case "NFE-REIMPRIMIR":
				...
				$sql = "SELECT xml, observacao FROM `nfe_".$filialGrupo."`.`NOTA_FISCAL` WHERE ".
				...
				if(!$CGerarDanfe->impressaoAutomatica($rContribuinte[0]['server_impressao'],$rContribuinte[0]['server_impressao_comando'])){
					$retornoJson['retorno']	 = false;
					$retornoJson['mensagem'] = $CGerarDanfe->mensagemErro;					
				}else{
					$retornoJson['retorno']	 = true;
					...
					$NfeUpdate = new MNotaFiscal($filialGrupo);
					$NfeUpdate->cnpj_emitente	= $_POST['hCnpj'];
					$NfeUpdate->ambiente		= $_POST['hAmbiente'];
					$NfeUpdate->numero_nota		= $_POST['hAtualizaNF'];
					$NfeUpdate->serie_nota		= $_POST['hAtualizaSerie'];
			...
			case "NFE-REIMPRIMIR-RANGE":
				...
				if($_POST['tReimprimirTipo'] == "P"){
					...
					// Selecionar notas autorizadas que estao no range do periodo informado
						...
						$sql = "SELECT chave, xml, numero_nota, observacao FROM `nfe_".$filialGrupo."`.`NOTA_FISCAL` WHERE ".
						" status = '03' AND ".
						" cnpj_emitente = '".$_POST['hCnpj']."' AND ".
						...
						// Loop para gerar todas as DANFEs das NFs retornadas no periodo
						$CGerarDanfe = new CGerarDanfe($filialGrupo);
			...
			// Verificar se tem inutilizadas
				...
				// Apenas 1 Arquivo, fazer do download dele unitariamente
                                echo "quantidade array:".count($arrayArquivosZip);
...
/*				if(!$zip->open($diretorioTemp.$nomeZip, ZIPARCHIVE::CREATE)){
					echo "VPortalMonitor.php: NFE-DOWNLOAD-XML -> Nao foi possivel criar o arquivo ZIP";
				}
	*/			
			...
			// Obter nota fiscal na tabela de notas
				...
				if($_POST['tReimprimirTipo'] == "P"){
					$periodoIni = substr($_POST['tReimprimirPeriodoInicial'],6,4)."-".substr($_POST['tReimprimirPeriodoInicial'],3,2)."-".substr($_POST['tReimprimirPeriodoInicial'],0,2);
					$periodoFim = substr($_POST['tReimprimirPeriodoFinal'],6,4)."-".substr($_POST['tReimprimirPeriodoFinal'],3,2)."-".substr($_POST['tReimprimirPeriodoFinal'],0,2);
					$sql = "SELECT numero_nota, xml, chave FROM `nfe_".$filialGrupo."`.`NOTA_FISCAL` WHERE ".
							" (status = '03' OR status = '06' OR status = '05') AND ".
							" data_emissao >= '".$periodoIni."' AND ".
				...
				if(is_array($returnNf)){
					foreach($returnNf as $conteudo){
						$arrayArquivosZip[$contArray]['xml'] = $conteudo['xml'];
						$arrayArquivosZip[$contArray]['nome'] = $conteudo['chave']."-procNFe.xml";
						$arrayArquivosZip[$contArray]['numero'] = $conteudo['numero_nota'];
						$contArray++;
					}
				}
			...
			// Verificar se tem inutilizadas
				...
				if($_POST['tReimprimirTipo'] == "P"){
					...
					$sql = "SELECT numero_nota_inicial, numero_nota_final, xml FROM `nfe_".$grupo."`.`INUTILIZACAO` WHERE".
				...
				if(is_array($returnInut)){
					foreach($returnInut as $conteudo){
						$arrayArquivosZip[$contArray]['xml'] = $conteudo['xml'];
						$arrayArquivosZip[$contArray]['nome'] = $conteudo['numero_nota_inicial']."-".$conteudo['numero_nota_final']."-procInut.xml";
						$arrayArquivosZip[$contArray]['numero'] = $conteudo['numero_nota_inicial'];
						$contArray++;
					}
				}
				...
				if($_POST['tReimprimirTipo'] == "P"){
					...
					$pSql = "SELECT e.xml, n.chave, e.tipo_evento FROM nfe_".$grupo.".EVENTO as e, nfe_".$grupo.".NOTA_FISCAL as n
							WHERE
							    n.data_emissao >= '".$periodoIni."'
							AND n.data_emissao <= '".$periodoFim."'
							AND e.`NOTA_FISCAL_cnpj_emitente` = ".$_POST['hCnpj']."
							AND e.`NOTA_FISCAL_ambiente` = ".$_POST['hAmbiente']."
					...
					$pSql = "SELECT e.xml, n.chave, e.tipo_evento FROM nfe_".$grupo.".EVENTO as e, nfe_".$grupo.".NOTA_FISCAL as n
							WHERE
								e.NOTA_FISCAL_numero_nota >= '".$_POST['tReimprimirNotaInicial']."'
							AND e.NOTA_FISCAL_numero_nota <= '".$_POST['tReimprimirNotaFinal']."'
							AND e.NOTA_FISCAL_cnpj_emitente = '".$_POST['hCnpj']."'
							AND e.NOTA_FISCAL_ambiente = '".$_POST['hAmbiente']."'
				...
				if(is_array($returnEvento)){
					foreach($returnEvento as $conteudo){
						...
						// Carta de Correcao
						if($conteudo['tipo_evento']=="6"){
							$arrayArquivosZip[$contArray]['nome'] = $conteudo['chave']."-procCCe";
						// Cancelamento
						}elseif($conteudo['tipo_evento']=="4"){
							$arrayArquivosZip[$contArray]['nome'] = $conteudo['chave']."-procCanc";
						}else{
							$arrayArquivosZip[$contArray]['nome'] = $conteudo['chave']."-procEvento";
						}
						$arrayArquivosZip[$contArray]['numero'] = $conteudo['NOTA_FISCAL_numero_nota'];
						$contArray++;
					}
				}
...
/*				if(!$zip->open($diretorioTemp.$nomeZip, ZIPARCHIVE::CREATE)){
	...
	*/
				...
				foreach($arrayArquivosZip as $conteudo){
					$nome = ($conteudo['nome'] != "") ? $conteudo['nome'] : $conteudo['numero'].".xml";
					$nome = ($nome == "") ? $cont: $nome.".xml";
					if(!file_put_contents($pastaTemp.$nome, base64_decode($conteudo['xml']))){
						/*$retornoJson['retorno']	 = false;
			...
			// Obter nota fiscal na tabela de notas
				...
				if($_POST['tReimprimirTipo'] == "P"){
					$periodoIni = substr($_POST['tReimprimirPeriodoInicial'],6,4)."-".substr($_POST['tReimprimirPeriodoInicial'],3,2)."-".substr($_POST['tReimprimirPeriodoInicial'],0,2);
					$periodoFim = substr($_POST['tReimprimirPeriodoFinal'],6,4)."-".substr($_POST['tReimprimirPeriodoFinal'],3,2)."-".substr($_POST['tReimprimirPeriodoFinal'],0,2);
					$sql = "SELECT chave, numero_nota, xml, observacao FROM `nfe_".$filialGrupo."`.`NOTA_FISCAL` WHERE ".
				...
				if(is_array($returnNf)){
					foreach($returnNf as $conteudo){
						$CGerarDanfe = new CGerarDanfe($filialGrupo);
						$CGerarDanfe->xmlNfe	= $conteudo['xml'];
						$CGerarDanfe->infAdic	= $conteudo['observacao'];
						if(!$CGerarDanfe->gerarDanfe()){
							$retornoJson['mensagem'] = $CGerarDanfe->mensagemErro;
						}else{
							$danfe[] = "/var/www/html/relatorios/".$CGerarDanfe->danfePdf;
							//update nota fiscal impressa
							$NfeUpdate = new MNotaFiscal($filialGrupo);
							$NfeUpdate->chave			= $conteudo['chave'];
							$NfeUpdate->danfe_impressa 	= "S";
							$NfeUpdate->update();
						}
					}
				...
				if(is_array($danfe)){
					if(count($danfe) > 1){
						$nomeRelatorio = "NFE_".date('Y-m-d_H-i-s').".pdf";
						echo system("php -q /user/objetos/JUGUNIPDF.php /var/www/html/relatorios/".$nomeRelatorio." ".implode(" ", $danfe)." > /dev/null");
						$retornoJson['arquivo'] = $nomeRelatorio;
						echo json_encode($retornoJson);
					}else{
						$retornoJson['arquivo'] = str_replace("/var/www/html/relatorios/","",$danfe[0]);
						echo json_encode($retornoJson);
					}
				}
	...
	// Tratar as fun��es que s�o provenientes do Contribuinte
	...
	function fContribuinte($pFuncao){
		...
		switch($pFuncao){
			case "CONTRIBUINTE-ACESSAR-REGISTRO":
				$CContribuinte->cnpj 		= $_POST['tFiliaisCNPJ'];
				$CContribuinte->ambiente 	= $_POST['tFiliaisNfeFlagProducao'];
				$return = $CContribuinte->mObterContribuinte();
				...
				if($retornoJson['retorno'] != ""){
					if($retornoJson['retorno'][0]['certificado_caminho'] != ""){
						$ToolsNFePHP = new ToolsNFePHP();
						$retornoJson['retorno'][0]['certificado_validade'] = $ToolsNFePHP->validadeCertificado($caminho.$retornoJson['retorno'][0]['certificado_caminho'],$retornoJson['retorno'][0]['certificado_senha']);
					}
				}
				echo json_encode($retornoJson);
				exit();
			break;
			case "CONTRIBUINTE-GRAVAR-REGISTRO": // Alterado os campos abaixo para colocar no SNF800
				// Seta variaveis do Contribuinte a serem gravadas
				$CContribuinte->cnpj 					= $_POST['tFiliaisCNPJ'];
				$CContribuinte->ambiente 				= $_POST['tFiliaisNfeFlagProducao'];
				$CContribuinte->uf 						= $_POST['tFiliaisNfeUF'];
				
				$CContribuinte->cod_emp_fil_softdib 	= $_POST['tFiliaisEmpresa'].$_POST['tFiliaisFilial'];
				$CContribuinte->razao_social 			= $_POST['tFiliaisNome'];
				$CContribuinte->pacote_xsd 				= $_POST['tFiliaisNfeXSD'];
				$CContribuinte->diretorio_integracao 	= $_POST['tFiliaisNfeIntegracao'];
				$CContribuinte->diretorio_backup 		= $_POST['tFiliaisNfeDiretorioBackup'];
				$CContribuinte->diretorio_importacao 	= $_POST['tFiliaisNfeDiretorioImportacao'];
				$CContribuinte->diretorio_base 			= $_POST['tFiliaisNfeBaseCliente'];
				$CContribuinte->danfe_layout_caminho 	= $_POST['tFiliaisNfeDanfeLayout'];
				$CContribuinte->danfe_logo_caminho 		= $_POST['tFiliaisNfeDanfeLogomarca'];
				$CContribuinte->danfe_automatica 		= $_POST['tFiliaisNfeDanfeAuto'];
				$CContribuinte->danfe_qtde_vias 		= $_POST['tFiliaisNfeDanfeVias'];
				$CContribuinte->server_impressao 		= $_POST['tFiliaisNfeDanfeServidor'];
				$CContribuinte->server_impressao_comando= $_POST['tFiliaisNfeDanfeServidorComando'];
				$CContribuinte->danfe_layout_fs_da 		= $_POST['tFiliaisNfeDanfeFsda'];
				$CContribuinte->email_smtp 				= $_POST['tFiliaisEmailSMTP'];
				$CContribuinte->email_porta 			= $_POST['tFiliaisEmailPorta'];
				$CContribuinte->email_ssl 				= $_POST['tFiliaisEmailConexao'];

				$CContribuinte->email_usuario			= $_POST['tFiliaisEmailUsuario'];
				$CContribuinte->email_senha 			= $_POST['tFiliaisEmailSenha'];
				$CContribuinte->email_remetente 		= $_POST['tFiliaisEmailUsuario'];
				$CContribuinte->email_conf_recebimento = "0";

				$CContribuinte->proxy_servidor 		= $_POST['tFiliaisProxyServidor'];
				$CContribuinte->proxy_porta 		= $_POST['tFiliaisProxyPorta'];
				$CContribuinte->proxy_usuario 		= $_POST['tFiliaisProxyUsuario'];
				$CContribuinte->proxy_senha 		= $_POST['tFiliaisProxySenha'];
				$CContribuinte->certificado_tipo 	= "A1";
				$CContribuinte->certificado_caminho	= $_POST['tFiliaisCNPJ'].".pfx";
				$CContribuinte->certificado_senha	= $_POST['tFiliaisWSSenha'];
				
				$CContribuinte->contigencia			= "01";

				$return = $CContribuinte->fGravar();
			...
			case "CONTRIBUINTE-ACESSAR-INICIAL":
				if(is_numeric($_POST['hCnpj'])){
					...
					$retornoJson['retorno']['warning'] = "";
					$retornoJson['retorno']['fatal'] = "";
					$retornoJson['retorno']['contigencia']	= "01";
					$retornoJson['retorno']['ambiente']		= $_POST['hAmbiente'];
					$retornoJson['mensagem']				= "";
				}
				echo json_encode($retornoJson);
				exit();
	...
	// Tratar as fun��es que s�o provenientes do Web Service
	...
	// Tratar as fun��es que s�o provenientes da Inutiliza��o
	...
	// Tratar as fun��es que s�o provenientes do Cancelamento
	...
	function fCancelar($pFuncao){
		...
		switch($pFuncao){
		  case "CANCELAR-NOTA":
			...
			$CCancelar->cnpjEmitente		= trim($contribuinte[0]);
			$CCancelar->ambiente			= trim($contribuinte[1]);
			$CCancelar->serieNota			= trim($_POST['tCancelarSerie']);
			$CCancelar->numeroNota			= trim($_POST['tCancelarNumero']);
		    ...
		    $return = $CCancelar->mCancelarNota();
		    ...
		    exit();
		  break;
		 case "CANCELAR-MOTIVO":
			$MEvento = new MEvento($filialGrupo);
			$MEvento->NOTA_FISCAL_cnpj_emitente = trim($contribuinte[0]);
			$MEvento->NOTA_FISCAL_numero_nota 	= trim($_POST['tMCancelarNumero']);
			$MEvento->NOTA_FISCAL_serie_nota 	= trim($_POST['tMCancelarSerie']);
			$MEvento->NOTA_FISCAL_ambiente 		= trim($contribuinte[1]);
			$MEvento->tipo_evento = "4";

			$return = $MEvento->selectMestre();

			$xml = base64_decode(base64_decode($return[0]['xml_env']));
			$doc = new DomDocumentNFePHP($xml);
			$justificativa = $doc->getElementsByTagName('xJust')->item(0)->nodeValue;
		    $retornoJson['mensagem']		= $MEvento->mensagemErro;
			$retornoJson['justificativa']	= $justificativa;
			echo json_encode($retornoJson);
		    exit();
		 break;
		}
	...
	// Tratar as fun��es que s�o provenientes de qualquer Servi�o (gen�rico para testar status do servi�o)
		...
		switch($pFuncao){
			case "SERVICO-STATUS":
				...
				switch($_POST['hContingencia']){
					case "03":
						$ToolsNFePHP = new ToolsNFePHP($contribuinte[0], $contribuinte[1],$filialGrupo,2,false, true); // O ultimo true � referente a contigencia SCAN
					break;
					case "06":
				...
				$resposta = $ToolsNFePHP->statusServico('','',$resp);
	...
	// Tratar as fun��es que s�o provenientes da Carta de Corre��o
	...
	// Tratar as fun��es que s�o provenientes da DANFE
	...
	function fDanfe($pFuncao){
		...
		switch($pFuncao){
			case "DANFE-IMPRIMIR":
				$MNotaFiscal = new MNotaFiscal($filialGrupo);
				/*if(trim($_POST['sContribuinte']) == "" || trim($_POST['tAutorizadaNF']) == "" || trim($_POST['tAutorizadaSerie']) == ""){
					$retornoJson['retorno']		= false;
					$retornoJson['mensagem']	= "Para visualizar a DANFE � necess�rio as informa��es: CNPJ, Ambiente, N�mero NF, S�rie NF!";
					echo json_encode($retornoJson);
					exit();
				}
				...
				$contribuinte = explode("-",$_POST['sContribuinte']);*/
				$chave = $_POST['hChave'];
				$retorno = $MNotaFiscal->selectAllMestre("SELECT xml, observacao FROM nfe_".$filialGrupo.".NOTA_FISCAL 
															WHERE chave = '".$chave."'");
				...
				$CGerarDanfe->xmlNfe	= $retorno[0]['xml'];
				$CGerarDanfe->infAdic	= $retorno[0]['observacao'];

				if(!$return = $CGerarDanfe->gerarDanfe()){
					$retornoJson['retorno']		= $return;
					$retornoJson['mensagem']	= $CGerarDanfe->mensagemErro;
					echo json_encode($retornoJson);
					exit();
				...
				// Update na nota fiscal
				$NfeUpdate = new MNotaFiscal($filialGrupo);
				$NfeUpdate->chave	= $chave;
				$NfeUpdate->danfe_impressa 	= "S";
				$NfeUpdate->update();

				$retornoJson['arquivo']	= $CGerarDanfe->danfePdf;
				echo json_encode($retornoJson);
				exit();
			break;
		}
	...
	// Tratar as fun��es que s�o provenientes da Consulta do Cadastro de Contribuinte ICMS
		...
		switch($pFuncao){
			case "CADCONT-CONSULTAR":
				if( (trim($_POST['tConCadCNPJ']) == "" && trim($_POST['tConCadIE']) == "" )
					...
					$retornoJson['retorno']		= false;
					$retornoJson['mensagem']	= "Para consulta � necess�rio informar o CNPJ ou IE e UF!";
					echo json_encode($retornoJson);
					exit();
				}
				...
				if(trim($_POST['sContribuinte']) == ""){
					$retornoJson['retorno']		= false;
					$retornoJson['mensagem']	= "Para consulta � necess�rio selecionar um Contribuinte!";
					echo json_encode($retornoJson);
					exit();
				}
				$contribuinte = explode("-",$_POST['sContribuinte']);
	...
	function fCritica($pFuncao){
		...
		switch($pFuncao){
			case "CRITICA-LISTAR":
				...
				$CNf->cnpj_emitente	= trim($_POST['hCriticaCnpj']);
				$CNf->numero_nota	= trim($_POST['hCriticaNumero']);
				$CNf->serie_nota	= trim($_POST['hCriticaSerie']);
				$CNf->ambiente		= trim($_POST['hCriticaAmbiente']);

				$return = $CNf->mObterCriticas();
	...
	function fLocalVerificarCertificado($pCertificadoCaminho, $pCertificadoSenha){
		...
		if($pCertificadoCaminho != ""){
			...
			$dataAtual = date('Ymd');
			$dataFutura = date('Ymd',strtotime('now + 10 days'));
