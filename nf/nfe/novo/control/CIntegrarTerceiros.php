...
*/		
	private function __TipoArquivo($pStringXml){
		if(	strpos($pStringXml, "<nfeProc") !== false &&
			strpos($pStringXml, "</nfeProc>") !== false ){
				return "nfeProc";
		}
		
		if(	strpos($pStringXml, "<procInutNFe") !== false &&
			strpos($pStringXml, "</procInutNFe>") !== false ){
				return "procInutNFe";
		}
		
/*		if(	strpos($pStringXml, "<retInutNFe") !== false &&
			strpos($pStringXml, "</retInutNFe>") !== false ){
				return "procInutNFe";
		}*/
		
		if(	strpos($pStringXml, "<procCancNFe") !== false &&
			strpos($pStringXml, "</procCancNFe>") !== false ){
				return "procCancNFe";
		}
		
		if(	strpos($pStringXml, "<procEventoNFe") !== false &&
			strpos($pStringXml, "</procEventoNFe>") !== false ){
				if(	strpos($pStringXml, "<tpEvento>110111</tpEvento>") !== false ){
	...
	*/
	private function __InserirNFe($pStringXml, $pArrayContribuinte){
		...
		if($doc->getElementsByTagName('tpAmb')->item(0)->nodeValue == "2"){
		...
		if($MNotaFiscal->versao == '3.10'){
			$MNotaFiscal->data_emissao		= $doc->getElementsByTagName('dhEmi')->item(0)->nodeValue;	
			$MNotaFiscal->data_entrada_saida= $doc->getElementsByTagName('dhSaiEnt')->item(0)->nodeValue." ".$doc->getElementsByTagName('hSaiEnt')->item(0)->nodeValue;
		}else{
