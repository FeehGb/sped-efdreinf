<?php
	...
	*/
	...
	/*
		...
		file_put_contents("/var/www/html/nf/xml_utf8.txt", $file_utf);
		exit();
	*/
	...
	switch($CIntegracaoERP->tipoArquivo){
        ...
        case "NFC":
			...
			if(!$retTools){
				...
				for($i = 0; $i < 20; $i++)
				{
					$xMotivo = str_replace("|", " ", $xMotivo);
				}

                $pArray = array('cnpj'=>$cnpj, 'ambiete'=>$ambiente, 'tpEmis'=>$tpEmis, 'cUF'=>$cUF, 'recibo'=>$retorno['infRec']['nRec'], 'chave'=>$chave, 'cStat'=>$retorno['cStat'], 'xMotivo'=>$xMotivo, 'dhRecebto'=>$retorno['dhRecebto'],'qrCode'=>$qrCode, 'xml'=>$retorno['xml'] );
				$CIntegracaoERP->mRetornoCobol($pArray,str_replace("NFC","LNFCR",$nomeArquivo));
			}else{
				...
				$mensagem = $nomeArquivo."|".$cnpj."|".$ambiente."|".$tpEmis."|".$cUF."|".$retorno['infRec']['nRec']."|".$chave."|pendente|0";
	...
	//		 print "\n".print_r($xml, true)."\n\n" ; exit();
			...
			//$xml_cobol = file_get_contents($argv[1]);
			//$xml_cobol = simplexml_load_string($xml_cobol, 'SimpleXMLElement', LIBXML_NOCDATA);
			...
			if(!$retTools){
				// TODO Gravar arquivo de erro na Caixa de Saida
                $pArray = array('cnpj'=>$cnpj, 'ambiete'=>$ambiente, 'tpEmis'=>$tpEmis, 'cUF'=>$cUF, 'recibo'=>$retorno['infRec']['nRec'], 'chave'=>$chave, 'cStat'=>$retorno['cStat'], 'xMotivo'=>$retorno['xMotivo'], 'dhRecbto'=>$retorno['dhRecbto'], 'xml'=>$retorno['xml'] );
				$CIntegracaoERP->mRetornoCobol($pArray,str_replace("NFE","LNFER",$nomeArquivo));
			}else{
				// Gravar arquivo de retorno para Cobol - /CaixaSaida/Sefaz
				$pArray = array('cnpj'=>$cnpj, 'ambiete'=>$ambiente, 'tpEmis'=>$tpEmis, 'cUF'=>$cUF, 'recibo'=>$retorno['infRec']['nRec'], 'chave'=>$chave, 'cStat'=>$retorno['cStat'], 'xMotivo'=>$retorno['xMotivo'], 'dhRecbto'=>$retorno['dhRecbto'], 'xml'=>$retorno['xml'] );
...
//                print_r($returnCC);
				...
				$CBackup = new CBackup();
//				echo "retornoXML\n";
//				print_r($ToolsNFePHP->arrayRetorno['xml']);
		...
		case "RNFE":
				...
				$arquivoloRetorno = str_replace("RNFC", "RNFCR", $arquivoloRetorno);
		...
		// Consulta Cadastro Cliente - CNPJ ou Incrição Estadual - 02/02/2017 ***** CONSULTA POR CPF NÃO ESTÁ FUNCIONANDO *****
		case "UNFE":

				$dadosArquivo = explode("|", $CIntegracaoERP->arquivoTXT);

			    $cnpj_contribuinte   = trim($dadosArquivo[0]);
			    $uf_contribuinte     = trim($dadosArquivo[1]);
			    $cnpj   			 = trim($dadosArquivo[2]);
			    $cUF    			 = trim($dadosArquivo[3]);
			    $tpEmis   			 = trim($dadosArquivo[4]);
			    $ambiente  	 		 = trim($dadosArquivo[5]);
			    $ie   				 = trim($dadosArquivo[6]);
			    $cpf  				 = trim($dadosArquivo[7]);

			    

	     		//$ToolsNFePHP = new ToolsNFePHP($cnpj_contribuinte, $ambiente, $uf_contribuinte, $tpEmis);
	     		$ToolsNFePHP = new ToolsNFePHP($cnpj_contribuinte, $ambiente, $cUF, $tpEmis);
	
				if(trim($cnpj) == "00000000000000") $cnpj = "";
				if(trim($ie) == "00000000000000") $ie = "";
				$resp = $ToolsNFePHP->consultaCadastro($cUF, $cnpj, $ie, $cpf, $ambiente);
				...
				{
					//echo $ToolsNFePHP->errMsg;
					$mensagem = "N|".$ToolsNFePHP->status_consulta."|||||||||||||".$ToolsNFePHP->xml_retorno."|";
				}
				else
				{
					$mensagem = "";
					for($i = 0; $i < 10; $i++)
					{
						if($resp["dados"][$i] != null && $resp["dados"][$i] != undefined)
						{
							$mensagem .= "S|";
							$mensagem .= $resp["cStat"]."|";	
							$mensagem .= count($resp["dados"])."|";
							$mensagem .= $resp["dados"][$i]["cSit"] . "|";
							$mensagem .= $resp["dados"][$i]["xNome"] . "|";
							$mensagem .= $resp["dados"][$i]["IE"] . "|";
							$mensagem .= $resp["dados"][$i]["xLgr"] . "|";
							$mensagem .= $resp["dados"][$i]["nro"] . "|";
							$mensagem .= $resp["dados"][$i]["xCpl"] . "|";
							$mensagem .= $resp["dados"][$i]["xBairro"] . "|";
							$mensagem .= $resp["dados"][$i]["cMun"] . "|";
							$mensagem .= $resp["dados"][$i]["CEP"] . "|";	
							$mensagem .= $resp["dados"][$i]["xMun"] . "|";
							$mensagem .= $resp["dados"][$i]["UF"] . "|";
							$mensagem .= $ToolsNFePHP->xml_retorno . "|\n";
						}
					}
				}

				if(!file_put_contents("/user/nfe/".$cnpj_contribuinte."/CaixaSaida/Sefaz/".str_replace("UNFE","UNFER",$nomeArquivo), $mensagem)){
                    echo "\n\nErro ao gravar arquivo de Integracao com ERP, verifique permissoes!\n\nArquivo: "."/user/nfe/".$cnpj_contribuinte."/CaixaSaida/Sefaz/".str_replace("UNFE","UNFER",$nomeArquivo)."\n\n";
                    return false;
            	}
