<?php
	...
	*/
	...
	require_once("libs/CC_ToolsNFePHP.class.php");
	require_once("control/CIntegracaoERP.php");
	require_once("control/CC_CBackup.php");
	...
	$file_get = str_replace("CaixaEntrada/Processado/CCNFE", "CaixaSaida/Sefaz/CCNFER", $nome_arquivo);

	//echo "\n".$file_get."\n";

	$proc_file = file_get_contents($file_get);
	$array_proc_file = explode("|", $proc_file);
	$xml_proc_file = end($array_proc_file);
	...
	switch($CIntegracaoERP->tipoArquivo){
        ...
        case "CCNFC":
				...
				$ToolsNFePHP = new ToolsNFePHP($CIntegracaoERP->CNPJ, $CIntegracaoERP->ambiente, $CIntegracaoERP->cUF, $CIntegracaoERP->tpEmis,0,false,false,false,$CIntegracaoERP->modelo);
                $returnCC = $ToolsNFePHP->envCCe($CIntegracaoERP->dhEvento, $CIntegracaoERP->chave, $CIntegracaoERP->descCorrecao, $CIntegracaoERP->sequencia);
                //print_r($returnCC);
				...
				$CBackup->mGuardarXml($xml_proc_file, $ToolsNFePHP->arrayRetorno['xml'], $ToolsNFePHP->arrayRetorno['xml_envio'], $CIntegracaoERP->chave, $CIntegracaoERP->CNPJ, "cc");
