...
*/
	...
	foreach($retorno as $pendente){
		...
		*/
		...
		$arrayRetorno['status'] = str_pad($status,2," ", STR_PAD_RIGHT); // Status da Nfe
		...
		//unset("temp/".$pendente['chave']."-pendNFe.xml");
		$xml = $ToolsNFePHP->addProt($xml, $aRetorno['xmlRetorno']);
                
		//Guarda o Backup da nota so se foi gerada com sucesso $status = 3
