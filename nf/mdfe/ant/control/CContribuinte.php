 ...
 * @description Classe elaborada para fazer a manutenчуo do cadastro de Contribuinte
 * @TODO 		Fazer tudo
*/
 ...
 * @TODO  Fazer as validaчѕes dos campos e chamar a Model para cadastrar as informaчѕes.
 */
	public function fGravar(){
		// Verifica se todos os campos pertinentes estуo setados
		$localMensagem = "";
		if(	trim($this->cnpj) == ""){ $localMensagem .= " CNPJ ";}
		if(	trim($this->ambiente) == ""){ $localMensagem .= " Ambiente ";}
		/*if(	trim($this->uf) == ""){ $localMensagem .= " Uf ";}			
		if(	trim($this->razao_social) == ""){ $localMensagem .= " Razуo Social ";}			
	...
	public function mAtualizarContingencia(){
		...
		// Em caso de SCAN, verificar se o mesmo estс ativo para operar
		...
		// Instancia Classe Model Contribuinte para atualizar a situaчуo
		$MContribuinte = new MContribuinte($this->grupo);
