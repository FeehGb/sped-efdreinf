...
require_once("../control/CContribuinte.php");
...
class ToolsNFePHP
{
    ...
    * @var string
    */
    public $raizDir='/var/www/html/nfe/novo/';
    /**
     * arqDir
...
/**
     ...
     * @var array
     */
    private $aliasConti =array('AC'=>'SVC-AN',
                               'AL'=>'SVC-AN',
                               'AM'=>'SVC-RS',
                               'AP'=>'SVC-AN',
                               'BA'=>'SVC-RS',
...
/**
     ...
     */
    function __construct($pCnpj='', $pAmbiente=0, $pGrupo='', $mododebug=2, $exceptions = false, $pScan=false){
		...
		if ($pGrupo == ''){
			$msg = "Parametro Grupo obrigatorio para comunicacao com SEFAZ .\n";
            $this->__setError($msg);
            if ($this->exceptions){
				throw new nfephpException($msg, self::STOP_CRITICAL);
            }
        ...
        // Estrutura de diretorios utilizados na manipulação das NFe
        ...
        // Arvore de diretórios necessária e estabelece permissões de acesso
		/*
		...
		*/
		...
		if($pScan != ""){
			...
			$this->aURL = $this->loadSEFAZ("/var/www/html/nf/nfe/novo/config/nfe_ws2.xml",$this->tpAmb,$pScan);
		}else{
			//$this->aURL = $this->loadWebServices($this->tpAmb,$this->UF);
			$this->aURL = $this->loadSEFAZ("/var/www/html/nf/nfe/novo/config/nfe_ws2.xml",$this->tpAmb,$this->UF);
		}
    ...
    */
    ...
    public function addProt($nfefile='', $protfile='') {
        try {
			...
			if(is_file($nfefile)){ $xmlnfe=file_get_contents($nfefile); }else{ $xmlnfe=$nfefile; }
			...
			if(is_file($protfile)){ $xmlprot=file_get_contents($protfile); }else{ $xmlprot=$protfile; }
    ...
    public function statusServico($UF='',$tpAmb='',$modSOAP='2', &$aRetorno=''){
        try {
            ...
            /* INFORMAÇÕES JÁ SETADAS NAS CONFIGURAÇÕES INICIAS
            ...
            /*if (!$this->enableSCAN){
                $aURL = $this->loadSEFAZ( $this->raizDir . 'config' . DIRECTORY_SEPARATOR . $this->xmlURLfile,$this->tpAmb,$this->UF);
            } else {
            ...
            $dados = '<nfeDadosMsg xmlns="'. $namespace . '"><consStatServ xmlns="'.$this->URLPortal.'" versao="'.$versao.'"><tpAmb>'.$this->tpAmb.'</tpAmb><cUF>'.$cUF.'</cUF><xServ>STATUS</xServ></consStatServ></nfeDadosMsg>';
...
//                        $nome = $doc->save($nome);
                    ...
                    if ($cStat == '104'){
                        ...
                        foreach ($protNfe as $d){
                            ...
                            if ( $protcStat == 100 || $protcStat == 110 || $protcStat == 301 || $protcStat == 302 ){
								...
								// SÓ FUNCIONARÁ SE FOR APENAS 1 NOTA FISCAL (GUILHERME)
     ...
     */
    public function inutNF($nAno='',$nSerie='1',$nIni='',$nFin='',$xJust='',$tpAmb='',$arrayRetorno=''){
        ...
        /*if(!file_put_contents($this->temDir.$id.'-pedInut.xml', $dXML)){ ver com BERTO
		...
		$cUF = !empty($doc->getElementsByTagName('cUF')->item(0)->nodeValue) ? $doc->getElementsByTagName('cUF')->item(0)->nodeValue : '';
        $cStat = !empty($doc->getElementsByTagName('cStat')->item(0)->nodeValue) ? $doc->getElementsByTagName('cStat')->item(0)->nodeValue : '';
		$dhRecbto = !empty($doc->getElementsByTagName('dhRecbto')->item(0)->nodeValue) ? $doc->getElementsByTagName('dhRecbto')->item(0)->nodeValue : '';
		...
		$arrayRetorno['cStat'] 		= $cStat;
		$arrayRetorno['dhRecbto']	= $dhRecbto;
        if ($cStat != '102'){
            //houve erro 
		...
		$arrayRetorno['xml_env'] 	= $dados;
		$arrayRetorno['xml_ret'] 	= $retorno;
		$arrayRetorno['xMotivo'] 	= $xMotivo;
		$arrayRetorno['cUF_env'] 	= $this->cUF;
		$arrayRetorno['cUF_ret'] 	= $cUF;
		$arrayRetorno['nProt'] 		= $nProt;

		//ABAIXO EH PRA RETORNAR O ARQUIVO APENDADO, NÃO FAZER POR ENQUANTO ATE VER A NECESSIDADE
       //gravar o retorno na pasta temp
	 ...
	 * TODO: AINDA FALTA FAZER O RETORNO POR ARRAY IGUAL DOS OUTROS
     */
    public function cancelEvent($chNFe='',$nProt='',$xJust='', $pLote='', $tpAmb='',$modSOAP='2'){
        try{
            ...
            //grava solicitação em temp
		/*            $arqName = $this->temDir."$chNFe-$nSeqEvento-eventCanc.xml"; NÃO HÁ NECESSIDADE
...
///				if($this->tpAmb == 2) $this->tpAmb=0;
			...
			$cStatLote = !empty($xmlretEvent->getElementsByTagName('cStat')->item(0)->nodeValue) ? $xmlretEvent->getElementsByTagName('cStat')->item(0)->nodeValue : '';
			$xMotivoLote = !empty($xmlretEvent->getElementsByTagName('xMotivo')->item(0)->nodeValue) ? $xmlretEvent->getElementsByTagName('xMotivo')->item(0)->nodeValue : '';
			...
			$nProt = !empty($retEnvEvento->getElementsByTagName('nProt')->item(0)->nodeValue) ? $retEnvEvento->getElementsByTagName('nProt')->item(0)->nodeValue : '';
			$dhRegEvento = !empty($retEnvEvento->getElementsByTagName('dhRegEvento')->item(0)->nodeValue) ? $retEnvEvento->getElementsByTagName('dhRegEvento')->item(0)->nodeValue : '';
			...
			$this->arrayRetorno['dhRegEvento']	= $dhRegEvento;
			$this->arrayRetorno['cUF']			= $cUF;
            ...
            if ($cStat == '238' || $cStat == '239'){
                $this->__trata239($retorno, $this->UF, $this->tpAmb, $servico, $versao);
            ...
            if ($cStat != 135 && $cStat != 155){
						// tá saindo aqui por erro de processamento
                //se cStat <> 135 houve erro e o lote foi rejeitado
                $msg = "Retorno de ERRO: $cStat - $xMotivo";
                throw new nfephpException($msg);
            }
			
			// Retorno do protocolo apendado
...
//file_put_contents("/var/guilherme/nfe/retornoCC.xml",$retorno);
			...
			$cStatLote = !empty($xmlretCCe->getElementsByTagName('cStat')->item(0)->nodeValue) ? $xmlretCCe->getElementsByTagName('cStat')->item(0)->nodeValue : '';
			$xMotivoLote = !empty($xmlretCCe->getElementsByTagName('xMotivo')->item(0)->nodeValue) ? $xmlretCCe->getElementsByTagName('xMotivo')->item(0)->nodeValue : '';
    ...
    */
    public function loadWebServices($tpAmb='',$sUF) {
        try {
			...
			if($tpAmb != "1"){ $MWebService->ambiente = "0"; }
			$MWebService->uf = $sUF;
...
//file_put_contents("/var/guilherme/nfe/entrada.".date("his"),$data);
...
//file_put_contents("/var/guilherme/nfe/retorno.".date("his"),$__xml);
    ...
    */
     ...
     *  
     * @name __setConfigurations (GUILHERME)
     * @param   none
     * @return  none
     */
    private function __setConfigurations(){

		$CContribuinte = new CContribuinte($this->grupo);
		$CContribuinte->cnpj = $this->cnpj;
		$CContribuinte->ambiente = $this->tpAmb;

		// consulta informações de ambiente na base de dados do contribuinte
		$retorno = $CContribuinte->mObterContribuinte();

		if(!$retorno){
            $this->__setError($this->mensagemErro);
            if ($this->exceptions){
				throw new nfephpException($msg, self::STOP_CRITICAL);
            }
            return false;
		...
		// converte valor da base de dados (0-homologação, 1-produção) para a aplicação (1-produção,2-homogação)
		if($retorno[0]['ambiente'] == "0"){
			$this->tpAmb = "2";
		}else{
			$this->tpAmb = "1";
		}
        $this->empName = $retorno[0]['razao_social'];
		$this->UF = $retorno[0]['uf'];
        $this->cUF = $this->cUFlist[$this->UF]; // Obter codigo da UF
        // $this->cnpj = $aConfig['cnpj']; Não há necessidade pois já esta setado
        $this->certName = $retorno[0]['certificado_caminho'];
        $this->keyPass = $retorno[0]['certificado_senha'];
        //$this->passPhrase = $aConfig['passPhrase']; Não é necessário por enquanto
        $this->arqDir = $retorno[0]['certificado_senha'];
        // $this->xmlURLfile = $aConfig['arquivoURLxml']; Não vai ser feito desta forma, será obtida no cadastro de WS
        // $this->URLbase = $aConfig['baseurl']; TODO obter a URL do site ou ver se pega de algum cadastro (fixo, etc)
		$this->danfeJrxml = $retorno[0]['danfe_layout_caminho'];
        $this->danfelogopath = $retorno[0]['danfe_logo_caminho'];
        $this->schemeVer = $retorno[0]['pacote_xsd'];
		
		// Verifica o tipo de emissão Normal,SCAN,DPEC,SVAN
		switch($retorno[0]['contigencia']){
			/* 	01 - NORMAL
				02 - FS
				03 - SCAN
				04 - DPEC
        ...
        if ($retorno[0]['proxy_servidor'] != '') {
            $this->aProxy = array('IP'=>$retorno[0]['proxy_servidor'],'PORT'=>$retorno[0]['proxy_porta'],'USER'=>$retorno[0]['proxy_usuario'],'PASS'=>$retorno[0]['proxy_senha']);
        }
        if ($retorno[0]['email_usuario'] != '') {
            $this->aMail = array('mailFROM'=>$retorno[0]['email_remetente'],'mailHOST'=>$retorno[0]['email_smtp'],'mailUSER'=>$retorno[0]['email_usuario'],'mailPASS'=>$retorno[0]['email_senha'],'mailPROTOCOL'=>"",'mailFROMmail'=>$retorno[0]['email_remetente'],'mailFROMname'=>$retorno[0]['email_remetente'],'mailREPLYTOmail'=>$retorno[0]['email_remetente'],'mailREPLYTOname'=>$retorno[0]['email_remetente']);
        }
