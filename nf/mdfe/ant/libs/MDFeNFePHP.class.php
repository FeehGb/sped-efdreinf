   ...
   /**
    ...
    function __construct($aConfig='',$mododebug=0,$exceptions=false) {
        ...
        if(is_array($aConfig)) {
            ...
            $this->certName = $aConfig['certificado_caminho'];
            $this->keyPass = $aConfig['certificado_senha'];
            $this->passPhrase = $aConfig['certificado_senha'];
            $this->arqDir = $aConfig['diretorio_integracao'];
            /*$this->URLbase = $aConfig['baseurl']; - nao vi necessidade destas variaveis
            ...
            $this->mdfeSchemeVer = $aConfig['pacote_xsd'];
...
file_put_contents("/home/guilherme/protocoloSaida.xml",$procXML);
    ...
    public function manifDest($chMDFe='',$tpEvento='',$tpAmb='',$cMun='', $modSOAP='2',&$resp='',$nProt=''){
        try {
            ...
            */
            ...
            $resp = array('bStat'=>true,'cStat'=>$cStat,'xMotivo'=>$xMotivo,'xml_env'=>$Ev,'xml_ret'=>$retorno,'xml'=>$procXML,'nProt'=>$nProt,'dhReceb'=>$dhReceb);
            //salva o arquivo xml
 ...
 // Retorno da funçao
        ...
        $cabec = '<mdfeCabecMsg xmlns="'. $namespace . '"><cUF>' . $cUF . '</cUF><versaoDados>' . $versao . '</versaoDados></mdfeCabecMsg>';
        // Montagem dos dados da mensagem SOAP
        $dados = '<mdfeDadosMsg xmlns="' . $namespace . '"><consMDFeNaoEnc xmlns="' . $this->URLPortal . '" versao="' . $versao . '"><tpAmb>' . $tpAmb . '</tpAmb><xServ>CONSULTAR NÃO ENCERRADOS</xServ><CNPJ>'.$CNPJ.'</CNPJ></consStatServMDFe></mdfeDadosMsg>';
...
file_put_contents("/var/www/html/nf/mdfe/temp/saida.txt",$__xml);
