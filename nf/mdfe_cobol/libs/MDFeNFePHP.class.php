   ...
   /**
    ...
    function __construct($aConfig='',$mododebug=0,$exceptions=false) {
        ...
        if(is_array($aConfig)) {
            ...
            /*if ($aConfig['proxy_servidor'] != '') {
                $this->aProxy =
                    array(
                        'IP' => $aConfig['proxy_servidor'],
                        'PORT' => $aConfig['proxy_porta'],
                        'USER' => $aConfig['proxy_usuario'],
                        'PASS' => $aConfig['proxy_senha']
                    );
            }
            if ($aConfig['email_usuario'] != '') {
                $this->aMAIL = array(
                        'mailFROM' => $aConfig['email_remetente'],
                        'mailHOST' => $aConfig['email_smtp'],
                        'mailUSER' => $aConfig['email_usuario'],
                        'mailPASS' => $aConfig['email_senha'],
                        'mailPROTOCOL' => $aConfig['email_porta'],
                        'mailFROMmail' => $aConfig['email_usuario'],
                        'mailFROMname' => $aConfig['email_usuario'],
                        'mailREPLYTOmail' => $aConfig['email_usuario'],
                        'mailREPLYTOname' => $aConfig['email_usuario']);
            }*/
        } else {
        ...
        $this->aURL = $this->loadSEFAZ($this->xmlURLfile, $this->tpAmb, 'RS'); // Fixo RS pq soh tem este
    ...
    public function validXML($xml='', $xsdFile='', $aError){
        ...
        if($xsdFile==''){
            ...
            /*
            echo "$this->xsdDir:".$this->xsdDir."\n\n";
            echo "$this->mdfeSchemeVer:".$this->mdfeSchemeVer."\n\n";
            */

            $aFile = $this->listDir($this->xsdDir . $this->mdfeSchemeVer. DIRECTORY_SEPARATOR,'mdfe_v*.xsd',true);
            //$aFile = $this->listDir($this->xsdDir . $this->mdfeSchemeVer. "PL_MDFe_100a",'mdfe_v*.xsd',true);


            if (!$aFile[0]) {
                $msg = "Erro na localizacao do schema xsd. ";
                $this->__setError($msg);
                if ($this->exceptions) {
...
echo "\n comeco6 \n";
    ...
    **/
    public function sendLot($aMDFe, $id="", $modSOAP = '2') {
        ...
        // Montagem dos dados da mensagem SOAP
        $dados = '<mdfeDadosMsg xmlns="' . $namespace . '"><enviMDFe xmlns="' . $this->URLPortal . '" versao="1.00"><idLote>' . $id . '</idLote>'. $sMDFe . '</enviMDFe></mdfeDadosMsg>';
...
file_put_contents("/home/guilherme/protocoloSaida.xml",$procXML);
            return $procXML;
    } // Fim addProt
    ...
    */
    ...
    protected function __sendSOAP($urlsefaz,$namespace,$cabecalho,$dados,$metodo,$ambiente,$UF=''){
        file_put_contents("/home/fernando/data.txt", "Entrou SOAP1");
       ...
       if ($this->enableSVAN){
        ...
        //$URL = $urlsefaz.'?WSDL';
        $URL = trim($urlsefaz).'?WSDL';
        $this->soapDebug = trim($urlsefaz);
        $options = array(
            'encoding'      => 'UTF-8',
            'verifypeer'    => false,
            'verifyhost'    => false,
        ...
        echo "antes instanciar\n";
        
        echo $URL."\n";
        print_r($options);
        file_put_contents("/home/fernando/data.txt", "\noptions:\n\t".print_r($options),FILE_APPEND);
        echo "\ndepois option\n";
    ...
    protected function __sendSOAP2($urlsefaz,$namespace,$cabecalho,$dados,$metodo,$ambiente,$UF=''){
        file_put_contents("/home/fernando/data.txt", "Entrou SOAP2");
        ...
        file_put_contents("/home/fernando/data.txt",$data);
        // file_put_contents("/home/fernando/data.txt", $parametros,FILE_APPEND);
        ...
        file_put_contents("/home/guilherme/mdfe.xml",$data."\n\n",FILE_APPEND);
        ...
        file_put_contents("/var/www/html/nf/mdfe_cobol/temp/saida.txt",$__xml);
