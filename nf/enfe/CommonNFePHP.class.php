...
class CommonNFePHP {
     ...
     * @return none
     */
    protected function __adicionaLogoPeloCnpj($CNPJ=''){
        if( $this->logomarca != '' ){
            return;
        }
    ...
    if($CNPJ==''){
        if(!empty($this->emit))
            $CNPJ=$this->emit->getElementsByTagName("CNPJ")->item(0);
        if(!empty($CNPJ))
            $CNPJ=$CNPJ->nodeValue;
        else
            return;
    }
        $imgPath = "logos/$CNPJ.jpg";
        ...
        //procurar diretorio acima do arquivo atual
        $imgPath = dirname(__FILE__)."/../logos/$CNPJ.jpg";
        if( file_exists( $imgPath ) ){
            $this->logomarca = $imgPath;
            return;
        }
