<?php
/*
	fonte: http://www.emissornfe.fazenda.sp.gov.br/v2/docs/[Emissor_NF-e]_Manual_de_layout_TXT-NF-e_v2.0.0.pdf
		e arquivos gerados pelo software emissor
*/ 
...
#die();
				...
				for($cur_item=0;$cur_item<$det->length;$cur_item++){
					...
					/* modificado por EDUARDO - 06/10/2014 */
					//$vTotTrib=trim($imposto->getElementsByTagName('vTotTrib')->item(0));
					$vTotTrib=$imposto->getElementsByTagName('vTotTrib')->item(0)->nodeValue;
				...
				if(!empty($infAdic)){
					...
					if(!empty($tmp_grupo)){
						for($c = 0; $c<$tmp_grupo->length; $c++){
							...
							if( !empty($xTexto) ){
								$xTexto=$xTexto->item(0)->nodeValue; // GJPS
