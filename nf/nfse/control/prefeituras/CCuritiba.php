...
/*
    function zulianLog($cur, $header)
    {
            ...
            $log .=                          $header ; 
            $log .= print_r(curl_getinfo($cur),true) ; 
            $log .= str_repeat("~", strlen($header)) ; 
            
            $path = "/home/luiz/logNFSE_" . date('Y-m-d') . ".txt" ; 
            file_put_contents( 
                    $path        , 
                    $log         , 
                    FILE_APPEND  ) 
    ...
    class CCuritiba{
        ...
        public function xmlEnvioCuritiba($xml_tve_cobol, $pStatus="")
        {
            ...
            do{
                ...
                $valorDeducao += str_replace(",",".",$xml_tve_cobol->itens->lista[$iten]->valor_deducao);
                $numeroDeducao = $xml_tve_cobol->itens->lista[$iten]->numero_deducao;
                
                ##t68809 - criado a codigoPrestacaoServico para recuperar  municipio  onde o servico foi executado...!
                $codigoPrestacaoServico = ltrim($xml_tve_cobol->itens->lista[$iten]->codigo_local_prestacao_servico);
            ...
            if((int)$numeroDeducao != 0){
                $ValorDeducao = $DomXml->createElement("ValorDeducoes", $valorDeducao); $Valores->appendChild($ValorDeducao);
                $NumeroDeducao = $DomXml->createElement("NumeroDeducao", $numeroDeducao); $Valores->appendChild($NumeroDeducao);
            ...
            #t68809 //$CodigoMunicipio = $DomXml->createElement("CodigoMunicipio", ltrim($xml_tve_cobol->prestador->cidade)); $Servico->appendChild($CodigoMunicipio);
            
            $CodigoMunicipio = $DomXml->createElement("CodigoMunicipio", $codigoPrestacaoServico); $Servico->appendChild($CodigoMunicipio);
        ...
        private function wsRespostaCuritiba($pChamada=""){
            ...
            do{
                ...
                //echo "\nxml que vai\n";
                  ...
                  if($pChamada == "COBOL")
                {
                    if(!$this->executarWS('ConsultarLoteRps', $pXmlConsulta, "N", '', "", "", "COBOL", $mensagemExtra)){
                        echo "\nerro1\n";
                          print_r($this->xmlRetornoWS);
                        return false;
                    }
                ...
                /*
                    LOG
                */
                $__doc = new DOMDocument('1.0')      ; 
                $__doc->preserveWhiteSpace = false   ; 
                $__doc->formatOutput = true          ; 
                $__doc->loadXML($this->xmlRetornoWS) ; 
                $xmlFormated= $__doc->saveXML()      ; 
                
                $ZmsgLog   = array();
                $ZmsgLog[] = "qtdeConsultas=$qtdeConsultas; ". date("d/m/Y H:i:s");
                $ZmsgLog[] = $xmlFormated;
                $ZmsgLog[] = str_repeat("~", 20) ; 
                $ZmsgLog   = implode("\n", $ZmsgLog);
                
                file_put_contents("/var/tmp/nfse.log", $ZmsgLog, FILE_APPEND);
                /*
                    LOG
                */
                ...
                echo "\n Protocolo: ".$this->retornoWS['protocolo']."\n";
                echo "\n codigoVerificacao: ".$this->codigoVerificacao."\n";
                

            }while(
                    (
                            trim($this->retornoWS['codVerificacao']) == "" 
                    ||  trim($this->retornoWS[      'numeroNF']) == ""
                    )
                    && (
                            // $qtdeConsultas <= 120
                            $qtdeConsultas <= 1000 // ~ Correcao ~ // 
                    )
            );
            // TODO: almentar o tempo apos o envio
            // sleep(30);
        ...
        private function executarWS($pMetodo, $pDados, $assinatura="N", $tagAssinada="", $noAssinar="", $tagAssinada2="", $pChamada="", $mensagemExtra=""){
            
            //!$this->executarWS('RecepcionarLoteRps', $CXml->xml, "S", 'LoteRps', "", "", "COBOL")){
            // !$this->executarWS('CancelarLoteRps', $xmlCancelamento, "S", "LoteRps", $pChamadaexecutarWS)){
          //!$this->executarWS('ConsultarLoteRps', $pXmlConsulta, "N", '', "", "", "COBOL", $mensagemExtra)
            ...
            else
            {
              ...
              {
                  //echo "\n\n CAssinaturaDigital:".$this->prestadorCNPJ."\n";
          ...
          $oCurl = curl_init();
            ...
            // testes
            curl_setopt($oCurl, CURLOPT_CONNECTTIMEOUT , 0); 
            curl_setopt($oCurl, CURLOPT_TIMEOUT        , 900); 
            // fim dos testes
          ...
          if($this->ambiente == "0"){
            ...
            curl_setopt($oCurl, CURLOPT_URL, "https://srv2-isscuritiba.curitiba.pr.gov.br/Iss.NfseWebService/nfsews.asmx?wsdl");
          }else{
            curl_setopt($oCurl, CURLOPT_URL, "https://piloto-iss.curitiba.pr.gov.br/nfse_ws/NfseWs.asmx");
            //curl_setopt($oCurl, CURLOPT_URL, "https://srv2-isscuritiba.curitiba.pr.gov.br/nfsews/nfsews.asmx?wsdl");
          }
