	...
	data: "funcao="+parmFuncao+"&"+$('#formPortal').serialize()
	  ...
	  switch(parmFuncao){
		  case "INICIAR":
			...
			fLocalListarContribuinte(json);
			fChamarAjax("LISTAR-NFS");
			$('#aguarde').hide();
		  break;
		  case "LISTAR-NFS":
			if(json.mensagemErro != undefined && json.mensagemErro != ""){
...
function fLocalListarContribuinte(pJson){
	var html = '<option value="" selected="selected">TODOS</option>';

	if(pJson.mensagemErro == ""){
		$('#sContribuinte').html(html);
		return false;
	}

	$.each(pJson.lista, function(i, item) { 
		cnpjMascara = pJson.lista[i].cnpj.substr(0,2)+"."+pJson.lista[i].cnpj.substr(2,3)+"."+pJson.lista[i].cnpj.substr(5,3)+"/"+pJson.lista[i].cnpj.substr(8,4)+"-"+pJson.lista[i].cnpj.substr(12,2);
		html += '<option value="'+pJson.lista[i].cnpj+'">'+cnpjMascara+' &nbsp; '+pJson.lista[i].descricao+'</option>';
	});
	
	$('#sContribuinte').html(html);
	
	$('#sContribuinte').unbind('change');
	$('#sContribuinte').change(function(){
		fChamarAjax("LISTAR-NFS");
	});
}
